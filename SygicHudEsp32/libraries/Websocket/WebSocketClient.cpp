...
bool WebSocketClient::analyzeRequest() {
    ...
    while (socket_client->connected() && !socket_client->available()) 
    {
      ...
      delay(250);
      yield();
      if (millis() - lBegin > 30000)
      {
        Serial.println("Websocket client - No response");
        return false;
      }
     }
...
void WebSocketClient::sendEncodedData(char *str, uint8_t opcode) {
    if (strlen(str) > 256-5)
    {
      Serial.print("ERROR: WebSocketClient::sendEncodedData: buffer too big\n");
      // too big buffer
      while (1);
    }
    ...
    *ptemp++ = opcode | WS_FIN;
    ...
    if (size > 125) 
    {
      *ptemp++ = WS_SIZE16 | WS_MASK;
      *ptemp++ = (uint8_t) (size >> 8);
      *ptemp++ = (uint8_t) (size & 0xFF);
    } else {
      *ptemp++ = (uint8_t) size | WS_MASK;
    }
    ...
    *ptemp++ = mask[3];     
    for (int i=0; i<size; ++i)
      *ptemp++ = str[i] ^ mask[i % 4];

    int n = (int)(ptemp-temp);
    socket_client->write(temp, n);
}
