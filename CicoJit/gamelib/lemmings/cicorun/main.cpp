...
#include "ega.h"
#include <list>
#include <unistd.h>//#include "game/game.h"
...
{
    if (x < 12)
        CicoContext::ctx->memory16(0x19a9, 0x10) = 8;
    else if (x >= 640-12)
        CicoContext::ctx->memory16(0x19a9, 0x10) = 640/2+8;
    else
        CicoContext::ctx->memory16(0x19a9, 0x10) = x/2+8;
    CicoContext::ctx->memory16(0x19a9, 0x12) = y/2+8;
}
...
void _sync()
{
    for (int y=0; y<CSdl::Height; y++)
      for (int x=0; x<CSdl::Width; x++)
...
//    CicoContext::ctx->memory8(0x8070, 0x8dc4) = p;
      //if (p)
//    CicoContext::ctx->memory16(0x19a9, 0x1a) = p ? 0xff : 0;
          switch (k)
          {
              case SDL_SCANCODE_1: p ? CicoContext::ctx->memory16(0x19a9, 0x14) |= 1 :
                  CicoContext::ctx->memory16(0x19a9, 0x14) &= ~1; break;
              case SDL_SCANCODE_2: p ? CicoContext::ctx->memory16(0x19a9, 0x14) |= 2 :
                  CicoContext::ctx->memory16(0x19a9, 0x14) &= ~2; break;
              case SDL_SCANCODE_3: p ? CicoContext::ctx->memory16(0x19a9, 0x14) |= 4 :
                  CicoContext::ctx->memory16(0x19a9, 0x14) &= ~4; break;
              case SDL_SCANCODE_4: p ? CicoContext::ctx->memory16(0x19a9, 0x14) |= 8 :
                  CicoContext::ctx->memory16(0x19a9, 0x14) &= ~8; break;

              case SDL_SCANCODE_Q: CicoContext::ctx->memory8(0x19a9, 0x1fd0) = p ? 0x2c : 0; break;
              case SDL_SCANCODE_W: CicoContext::ctx->memory8(0x19a9, 0x1fd0) = p ? 0x2d : 0; break;
...
void cicocontext_t::_int(int i)
{
    ...
    if (i == 0x21 && ctx->a.r8.h == 0x3f)
    {
        ...
        assert(files[ctx->b.r16-10]);
        if (!files[ctx->b.r16-10])
        {
...
//            printf("Read %d bytes -> %04x:%04x\n", ctx->c.r16, ctx->_ds, ctx->d.r16);
            ...
            int c = fread(buf, 1, (size_t)ctx->c.r16, files[ctx->b.r16-10]);
            if (ctx->_ds >= 0xa000)
            {
                //printf("cfg=%d\n", mEga.cfgDataRotate);
...
//            assert(c);
    ...
    if (i == 0x21 && ctx->a.r8.h == 0x3e)
    {
        ...
        fclose(files[ctx->b.r16-10]);
        files[ctx->b.r16-10] = nullptr;
        ctx->carry = 0;
        return;
    }
...
//        static int freeMem = 0x2800;
    ...
    if (i == 0x21 && ctx->a.r8.h == 0x42)
    {
        ...
        {
            assert(files[ctx->b.r16-10]);
            fseek(files[ctx->b.r16-10], 0L, SEEK_END);
            ctx->a.r16 = ftell(files[ctx->b.r16-10]);
            fseek(files[ctx->b.r16-10], 0L, SEEK_SET);
            ctx->d.r16 = 0;
            return;
        }
        if (ctx->a.r8.l == 0)
        {
            assert(files[ctx->b.r16-10]);
            fseek(files[ctx->b.r16-10], ctx->c.r16*0x10000 + ctx->d.r16, SEEK_SET);
            ctx->carry = false;
            return;
        }
    ...
    if (i == 0x21 && ctx->a.r8.h == 0x42)
    {
        ...
        if (ctx->a.r8.l == 1)
        {
            assert(ctx->c.r16 == 0);
            assert(files[ctx->b.r16-10]);
            fseek(files[ctx->b.r16-10], ctx->d.r16, SEEK_CUR);
            ctx->d.r16 = ftell(files[ctx->b.r16-10]);
            ctx->a.r16 = 0;
            return;
        }
        ...
        {
            assert(files[ctx->b.r16-10]);
            fseek(files[ctx->b.r16-10], ctx->c.r16, SEEK_CUR);
            return;
        }
        /*
        if (ctx->a.r8.l == 0)
