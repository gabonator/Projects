...
{
  cicocontext_t ctx;
  uint8_t memoryPspGet8(int seg, uint16_t ofs)
  {
    return 0;
  }
  ...
  void cicocontext_t::callInterrupt(int i)
  {
    ...
    static uint32_t keyBuffer = 0x3131;
    if (i == 0x1a || i == 0x60 || i == 0x61)
        return;
    if (i == 0x10)
    {
    ...
    {
        ctx.a.r16 = ctx._endAddress;
        ctx._endAddress += ctx.b.r16;
        ctx._endAddress++;
        assert(ctx._endAddress*16 < sizeof(ctx.memory));
        ctx.carry = false;
        return;
    }
    ...
    if (i == 0x21 && ctx.a.r8.h == 0x02)
        return; // display char
    if (i == 0x21 && ctx.a.r8.h == 0x07)
    {
        ctx.a.r8.l = keyBuffer & 0xff;
        keyBuffer >>= 8;
        return;
    }
