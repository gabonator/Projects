...
//            assert(0);
        ...
        if (ctx->a.r16 == 0x1012)
        {
            for (int i=0; i<ctx->c.r16; i++)
            {
                ...
                palette[palIndex] = b | (g << 8) | (r << 16) | 0xff000000;
            }
            return true;
        }
        if (ctx->a.r16 == 0x1002)
        ...
        if (ctx->a.r8.h == 0x0f || ctx->a.r8.h == 0x02)
        {
            return true;
        }
{
char tmp[32];
sprintf(tmp, "EGA ax=0x%x", ctx->a.r16);
apiPrint(tmp);
}
//        assert(0);
        return false;
...
{
char tmp[64];
sprintf(tmp, "EGA port16[0x%x]=0x%x", port, data);
apiPrint(tmp);
}
...
//  modified = false;
    ...
    { 
        ...
        planes += (2 + rowOffset*2*16)*4;
        for (int x=0; x<200; x++)
        {
            uint32_t* pixels2 = pixels + 640;
            for (int y=0; y<320/8; y++)
            {
                int plane0 = *planes++;
                int plane1 = *planes++;
                int plane2 = *planes++;
                ...
                for (int i=0; i<8; i++)
                {
                    ...
                    plane2 <<= 1;
                    plane3 <<= 1;
                    uint32_t color = palette[index >> 7];
                    *pixels++ = color;
                    *pixels++ = color;
                    *pixels2++ = color;
                    *pixels2++ = color;
