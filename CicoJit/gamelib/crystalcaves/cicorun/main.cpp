...
{
    printf("indi: %04x:%04x - sub_%x()\n", s, o, s*16+o);
    switch(s*16+o)
    {
        case 0x2e6e7: sub_2e6e7(); return;
        case 0x2a769: sub_2a769(); return;
    }
//
//    switch (a)
//    {
...
uint8_t memoryBiosGet8(int seg, int ofs)
{
    ...
    if (seg == 0x0000 && ofs == 0x0484)
        return 40;
    if (seg == 0x0000 && ofs >= 0xff63 && ofs < 0xff73)
    {
        uint8_t buf[] = {0x83, 0x3e, 0x2e, 0x51, 0x00, 0x7e, 0x23, 0xa1, 0x2e, 0x51, 0x05, 0x96, 0x00, 0x3b, 0x06, 0x11};
        return buf[ofs-0xff63];
    }
    assert(0);
    return 0;
}
...
uint8_t& cicocontext_t::memory8(int seg, int ofs){
    if (seg*16+ofs == 0x10400+0x9709+15)
    {
...
//            assert(c);
    ...
    if (i == 0x10 && ctx->a.r16 == 0x1c00)
    {
        // http://www.techhelpmanual.com/113-int_10h__video_services.html
        // save restore video funcs
        // query size of video buf
        printf("VGA Query Size of State-Save Buffer cx=%x\n", ctx->c.r16);
        ctx->a.r8.l = 0x1c;
        ctx->b.r16 = 0x10;
        return;
    }
...
void cicocontext_t::load(const char* path, const char* file, int size)
{
    ...
    loadAddress = this->_loadAddress; //0x1000; // - 0x0130; //CicoContext::ctx->_cs;
}
