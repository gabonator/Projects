...
apiPrint(temp);
  ...
  {
//char temp[128];
//sprintf(temp, "memoryBiosSet8(%04x:%04x) = 0x%x", seg, ofs, v);
//apiPrint(temp);
  }
  void cicocontext_t::memoryBiosSet16(int seg, uint16_t ofs, uint16_t v)
  {
//char temp[128];
//sprintf(temp, "memoryBiosSet16(%04x:%04x) = 0x%x", seg, ofs, v);
//apiPrint(temp);
  }
  uint8_t cicocontext_t::memoryBiosGet8(int seg, uint16_t ofs)
  {
    ...
    if (seg == 0x0040 && ofs == 0x006c)
        return mFrameCounter*2; // number of ticks since midnight 54ms
    ...
    char temp[128];
    sprintf(temp, "unhanled bios read8 %04x:%04x", seg, ofs);
    apiPrint(temp);
    assert(0);
    return 0;
  }
  uint16_t cicocontext_t::memoryBiosGet16(int seg, uint16_t ofs)
  {
    ...
    if (seg == 0x0040 && ofs == 0x006c) // timer
        return mFrameCounter*2;

    return 0;
  }
  uint8_t cicocontext_t::memoryPspGet8(int seg, uint16_t ofs)
  {
char temp[128];
sprintf(temp, "memoryPspGet8(%04x:%04x)", seg, ofs);
apiPrint(temp);

    return 0;
  }
  uint16_t cicocontext_t::memoryPspGet16(int seg, uint16_t ofs)
  {
    return 0;
char temp[128];
sprintf(temp, "memoryPspGet16(%04x:%04x)", seg, ofs);
apiPrint(temp);
  }
