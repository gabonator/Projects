...
uint8_t memoryPsp(int seg, int ofs)
{
    ...
    if (seg == 0x1dd && ofs == 0x81)
        return 0x0d;
    if (seg == 0x1ed && ofs < 0x200)
    {
        return CicoContext::ctx->memory8(seg, ofs);
    }
...
uint16_t memoryBiosGet16(int seg, int ofs)
{
    ...
    {
        // https://github.com/joncampbell123/dosbox-x/blob/master/docs/BDA%20-%20BIOS%20Data%20Area%20-%20PC%20Memory%20Map.txt
...
//    assert (seg >= 0x01ed && seg < 0xa000);
//    assert(ofs >= 0 && ofs <= 0xffff);
    if (seg*16+ofs == 0x27660+0x195 && val != 0xe0 && val != 0)
    {
        int f = 9;
    }
    if (seg != 0xb000 && seg != 0x0ac3 && val == 0x4c)
    {
        int f = 9;
    }
    if (seg == 0x22a6 /*&& ofs >= 0x4c96 && ofs <= 0x4e96 && val == 0x4c*/ && val == 0x4c)
    {
        int f = 9;
    }
    ofs &= 0xffff;
    if (seg*16+ofs==0xc410+0x0eea)
    {
        int f = 9;
    }
    if ((seg == 0x01dd /*|| seg == loadAddress*/) && ofs < 256)
...
//    assert(ofs >= 0x0000 && ofs <= 0xffff);
    ...
    if (seg*16+ofs==0xac3+0x1307) // bpm ac3:1307
    {
        int f = 9;
    }

    if (seg*16+ofs == 0x0ac30+0x1696)
    {
        int f = 9;
    }
    ofs &= 0xffff;
...
//        mText.SetPaletteIndex(ctx->b.r8.l, ctx->b.r8.h);
//        return;
//    }
//    if (i == 0x10 && ctx->a.r16 == 0x1012)
//    {
...
//    if (i == 0x10  && ctx->a.r8.h == 0x03)
//        return;
//    if (i == 0x10  && ctx->a.r8.h == 0x09)
//        return;
//    if (i == 0x10  && ctx->a.r8.h == 0x02)
//        return;
...
//    memory16(0x1000, 0x35e2)++;
    _sync();
    //CicoContext::ctx->memory16(0xc41, 0x0edb)++;
        //CicoContext::ctx->memory16(0xc41, 0x0edd)++;
//    CicoContext::ctx->memory16(0xc41, 0x0c71)++;
//    CicoContext::ctx->memory16(0xc41, 0x0c71+2)++;
//    CicoContext::ctx->memory16(0xc41, 0x0c71+4)++w;
    //CicoContext::ctx->memory8(0xc41, 0x0c71) |= 4;
}
