...
//if (es == 0x3824 && di < 0x1000)
    if (callcnt == 4 && es == 0x3824 && cnt++ < 1000 /*&& (cnt++ % 100) == 0*/)
    sprintf(tmp, "wrsc %04x:%04x = %02x", es, di, al);
    apiPrint(tmp);
...
loc_13c9d: // 1000:3c9d
    ...
    sub_15b21();
    sub_10d5f();
    memoryASet16(ds, 0x991c, 0x0001); // level
...
void sub_208dc() // 1f29:164c draw score
{
    CStackGuardFar sg(0, false);
    di = 0x1e03;
    if (memoryAGet16(ds, 0x8e7a) == 0x0000)
...
loc_20963: // 1f29:16d3
    lodsb<MemAuto, DirAuto>();
sprintf(temp, "rdsc %04x:%04x = %02x", ds, si, al);
apiPrint(temp);
al = 0xff; // gabo score
    memoryASet(es, di, al);
