...
void cicocontext_t::overlay(const char* path, const char* file, int size, int seg, int offset)
{    
    while (offset<0)
    {
        offset += 0x10;
    ...
    sprintf(fpath, "%s/%s", path, file);
    FILE* f = fopen(fpath, "rb");
    fread(&memory8(seg, offset), size, 1, f);
    fclose(f);
}
