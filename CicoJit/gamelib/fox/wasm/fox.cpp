...
bool foxInfiniteEnergy = true;
bool foxInfiniteLives = false;
int foxStartupLevel = 0;
bool foxSkipIntro = true;
...
void start()
{
    ...
    memoryASet(0x168f, 0x5250, foxStartupLevel);
    sub_10205();
}
void sub_10200();
void sub_10205();
...
loc_1024f:
    ...
    ax = 0;
    ds = ax;
    memoryBiosSet16(ds, 0x0090, 0x08d0);
    memoryBiosSet16(ds, 0x0092, cs);
    ds = pop();
    sub_119fe();
    ax = 0x168f;
    ds = ax;
...
void sub_10eeb()
{
    ...
    flags.direction = false;
loc_10f05:
    ax = memoryBiosGet16(es, di);
    tx = memoryAGet16(ds, si);
    memoryASet16(ds, si, ax);
    ax = tx;
    stosw<MemBios, DirAuto>();
    si++;
    si++;
    if (--cx)
        goto loc_10f05;
...
void sub_1127d()
{
    ...
    ax = 0;
    es = ax;
    push(memoryBiosGet16(es, 0x0018));
    push(memoryBiosGet16(es, 0x001a));
    memoryBiosSet16(es, 0x0018, 0x10bc);
    memoryBiosSet16(es, 0x001a, cs);
    al = 0x01;
    cl = 0x20;
    al <<= cl;
    if (al == 0x01)
...
loc_112b1:
    ...
    //   gap of 9 bytes
loc_112c5:
    memoryBiosSet16(es, 0x001a, pop());
    memoryBiosSet16(es, 0x0018, pop());
    flags.interrupts = true;
    es = pop();
    ds = pop();
}
