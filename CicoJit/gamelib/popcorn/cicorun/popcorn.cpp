...
loc_2c66a:
  ...
  if (--cx)
    goto loc_2c66a;
  callIndirect(memoryAGet16(ds, 0x2d45));
...
loc_2c777:
  if (bx == 0xffff)
    goto loc_2c79b;
  push(bx);
  callIndirect(memoryAGet16(ds, bx));
  bx = pop();
...
// INJECT: Error: cannot inject carry flag in sub_2d447()!
void sub_2d447()
{
  bp = si;
  si += 0x0004;
...
//    tl = memoryAGet(es, di);
//    tl = rcl(tl, 1); todo!
  memoryASet(es, di, rcl(memoryAGet(es, di), 0x01));
  di--;
  bl--;
  if (bl != 0)
    goto loc_2fd6f;
