...
namespace CicoContext
{
  ...
  uint16_t cicocontext_t::memoryBiosGet16(int seg, int ofs)
  {
    ...
    char temp[128];
    sprintf(temp, "cicocontext_t::memoryBiosGet16 rd %04x:%04x", seg, ofs);
    apiPrint(temp);
    return 0;
  }
  ...
  void cicocontext_t::callInterrupt(int i)
  {
    ...
    char temp[128];
    sprintf(temp, "interrupt 0x%02x ah=0x%02x al=0x%02x", i, ctx.a.r8.h, ctx.a.r8.l);
    apiPrint(temp);
...
//            sync();
    ...
    {
        if (ctx._ds >= 0xa000/* && ctx.d.r16 == 0*/)
        {
            uint8_t temp[1024];
            int totalRead = ctx.c.r16;
            int shift = mVideo.cfgReadMapSelect*8;
            uint32_t mask = ~(0xff << shift);
            int ofs = ctx.d.r16;
            ctx.a.r16 = 0;
            while (totalRead)
            {
                int toRead = totalRead < sizeof(temp) ? totalRead : sizeof(temp);
                int readBytes = apiRead(currentFile, currentOfs, toRead, temp);
                assert(readBytes == toRead);
                for (int i=0; i<readBytes; i++)
                    mVideo.egamemory[ofs++] &= mask;
                ofs -= readBytes;
                for (int i=0; i<readBytes; i++)
                    mVideo.egamemory[ofs++] |= temp[i]<<shift;
...
//        keyBuffer.pop_front();
    ...
    {
        while (CaptainLastKey == -1)
            sync();
        ctx.a.r16 = CaptainLastKey;
        CaptainLastKey = -1;
        return;
    }
