...
class CEgaVgaBgi : public CVideoAdapter
{
    uint32_t palette[16] = {
        0x000000, 0x0000b0, 0x00b000, 0x00b0b0, 0xb00000, 0xb000b0, 0xb0b000, 0xb0b0b0,
        0x808080, 0x0000ff, 0x00ff00, 0x00ffff, 0xff0000, 0xff00ff, 0xffff00, 0xffffff};

    uint8_t video[640*480];
    int color{15};
    int x, y;
    virtual bool PortWrite16(int port, int data) { assert(0); }
    ...
    virtual bool Interrupt(CicoContext::cicocontext_t* ctx)
    {
        switch (ctx->_si)
        {
            ...
            case 0x1e:
                ...
                break;
            case 0x0c:
                line(ctx->a.r16, ctx->b.r16, ctx->c.r16, ctx->d.r16, color & 0x0f);
                break;
            case 0x32:
                //printf("bitmap util\n");
                // es:bx -> method table
                ...
                break;
            case 0x2e:
                if (ctx->b.r16 >= 0 && ctx->b.r16<480 && ctx->a.r16>=0 && ctx->a.r16<640)
                    ctx->d.r8.l = video[ctx->b.r16*640+ctx->a.r16];
                break;
            case 0x30:
                SetPixel(ctx->a.r16, ctx->b.r16, ctx->d.r8.l);
                {
                    static int n = 0;
                    if (n++ % 20 == 0)
                        _sync();
            ...
            case 0x26:
                printf("text %d,%d, c: 0x%02x '%s'\n", x, y, color, &ctx->memory8(ctx->_es, ctx->b.r16));
                break;

/*
 printf("EGAVGA %04x: ", si, ax);
 switch (si)
 {
     case 0:
     {
     ...
     case 0x32:
         ...
         break;
     case 0x3a:
         printf("color query %d\n", al);
         break;
     case 0x0c:
         printf("line %d,%d-%d,%d\n", ax, bx, cx, dx);
         break;
     case 0x1a:
         printf("set palette %x - %x, %x, %x\n", ax, bx, cx, dx);
         break;
     case 0x1c:
         printf("all palette %04x:%04x\n", es, bx);
         break;
     case 0x20:
     case 0x22:
     case 0x24:
     ...
     case 0x2e:
         ...
         break;
     case 0x30:
         printf("setpixel %d,%d,%d\n", (short)ax, (short)bx, dl);
         sync();
         break;
         // 0x30, 0x25 get pixel, set pixel
     case 0x38:
    ...
    virtual void Write(uint32_t dwAddr, uint8_t bWrite) {/* assert(dwAddr < 0xb800 + 80*25*2); */}
    virtual uint8_t Read(uint32_t dwAddr) { assert(0); }
    virtual uint32_t GetPixel(int x, int y)
    {
        return palette[video[y*640+x]];
    }
...
CEgaVgaBgi mEgaVgaBgi;
CVideoAdapter* mVideo = &mText;
int lastKey = -1;
char root[1024];
uint8_t mem[0xa0000];
...
uint8_t memoryBiosGet8(int seg, int ofs)
{
    ...
    if (seg == 0x0040 && ofs == 0x0012) 
        return 0x0;
    if (seg == 0x0040 && ofs == 0x004a)
        return 0x50;
    if (seg == 0x0040 && ofs == 0x0049)
        return 0x5003;
    if (seg == 0x0040 && ofs == 0x0000)
        return 0xf8;
    assert(0);
    return 0;
}
uint8_t memoryPsp(int seg, int ofs)
{
    if (seg == 0x1d3)
    {
        const char env[] = "PATH=Z:\\" "\x00" "COMSPEC=Z:\\COMMAND.COM" "\x00"
          "BLASTER=A220 I7 D1 H5 T6" "\x00\x00\x01\x00" "C:\\COS\\COSMO1.EXE"
...
{
//    printf("skip bios write %x:%x = %x\n", seg, ofs, v); //?
    if (seg == 0x0040 && ofs == 0x0050) // cursor pos!
    {
        mText.posy = v >> 8;
        mText.posx = v & 0xff;
    }
...
uint16_t memoryBiosGet16(int seg, int ofs)
{
    ...
    if (seg == 0x0040 && ofs == 0x0050) // cursor pos!
    {
        return (mText.posy << 8) | mText.posx;
    }
    assert(0);
    return 0;
}
...
//        ctx->b.r16 = 3;
    ...
    if (i == 0x21  && ctx->a.r8.h == 0x6)
    {
        printf("%c", ctx->a.r8.l);
        return;
    }
    if (i == 0x10  && ctx->a.r8.h == 0x8)
    {
        ctx->a.r16 = 0x0720;
        return;
    }
...
void cicocontext_t::imul(uint16_t& r, uint16_t a, uint16_t b)
{
    int16_t sa = a;
    int16_t sb = b;
//    uint32_t m = sa*sb;
//    r = m;
//    ctx->d.r16 = 
//    assert((int16_t)a*(int16_t)b == (int16_t)(a*b));
    r = a*b;
}
