...
loc_5bdb:
    ...
    bx = ax;
    bx <<= 1;
    stop(/*2*/); // 01ed:3d21 jmp word ptr cs:[bx + 0x3d26] - jump Indirect memoryAGet16(cs, bx + 15654)
    //   gap of 74 bytes
loc_5c40:
...
//    stop(/*2*/); // 01ed:4adc jmp word ptr cs:[0x49f0] - jump Indirect memoryAGet16(cs, 0x49f0)
}
// INJECT: Error: cannot inject flag in sub_694f() because of being label!
void sub_694f()
{
...
//    stop(/*2*/); // 01ed:568b jmp word ptr cs:[bx + 0x5690] - jump Indirect memoryAGet16(cs, bx + 22160)
    //   gap of 20 bytes
loc_7574:
    bx = memoryAGet16(ss, bp + 14);
    es = memoryAGet16(ss, bp + 14 + 2);
...
void sub_7813()
{
    ...
    bx = ax;
    bx <<= 1;
    stop(/*2*/); // 01ed:596a jmp word ptr cs:[bx + 0x596f] - jump Indirect memoryAGet16(cs, bx + 22895)
    //   gap of 78 bytes
loc_788d:
    if (si == 0x0001)
        goto loc_7897;
...
loc_929b:
    ...
    al <<= 1;
    di = ax;
    stop(/*2*/); // 08e6:0441 jmp word ptr [di + 0x445] - jump Indirect memoryAGet16(ds, di + 1093)
}
// INJECT: Error: cannot inject carry flag in sub_9dbc()!
void sub_9dbc()
{
...
loc_a2a5:
    ...
    ah = 0;
    di = ax;
    stop(/*2*/); // 08e6:1450 jmp word ptr [di + 0x1454] - jump Indirect memoryAGet16(ds, di + 5204)
    //   gap of 101 bytes
loc_a319:
    push(es);
    push(di);
