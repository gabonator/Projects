...
public:
	...
	CSerializer& operator <<(const string& str)
	{
		...
		if (pInput)
		{
			...
			buf[nMaxLength-1] = 0;
            string& strx = (string&)str;
			strx = string(buf, strlen(buf));
		}
		return *this;
	}
	...
	CSerializer& operator <<(const CSerializable& src)
	{
        CSerializable& _src = (CSerializable&)src;
		_src.Serialize(*this);
		return *this;
	}
};
...
template<typename T>
    void _shared<T>::Serialize(CSerializer& s)
    {
        if ( s.IsWriting() )
        {
