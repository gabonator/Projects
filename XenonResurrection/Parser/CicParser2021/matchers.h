...
class CIMFixedArgOp : public CInstructionMatcher
{
	virtual shared_ptr<CInstruction> Match(string strLine) override
	{
		...
		if ( CUtils::match("^(cli|sti|std|stc|ctc|cld|aaa|cbw|lodsw|lodsb|stosb|stosw|movsw|movsb|cmpsb|clc|sahf|lahf|popf|pushf|xlat cs|xlat ds|xlat es|cmc|scasb)$", strLine, arrMatches) )
		{
