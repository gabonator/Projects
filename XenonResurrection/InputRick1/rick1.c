...
loc_000b:                                       //loc_000b:
    ...
    if (_FIXME_)                                //jz loc_000b
        goto loc_000b;
    sub_1b33();                                 //call sub_1b33
loc_0013:                                       //loc_0013:
    sub_0433();                                 //call sub_0433
    if (_FIXME_)                                //jz loc_0013
        goto loc_0013;
    _ax = 0x1040;                               //mov ax, 0x1040
    _ds = _ax;                                  //mov ds, ax
    _es = _ax;                                  //mov es, ax
...
loc_02a2:                                       //loc_02a2:
    ...
    _al = _al + memory(_ds, _si) + _flags.carry; _ASSERT(0);
...
loc_02b0:                                       //loc_02b0:
    ...
    if (--_cx)                                  //loop loc_02a2
        goto loc_02a2;
    _al = memory(_ds, _di);                     //mov al, byte ptr [di]
    _al = _al + 0x00 + _flags.carry; _ASSERT(0);//adc al, 0x0
    memory(_ds, _di) = _al;                     //mov byte ptr [di], al
    _ds = _pop();                               //pop ds
    {sub_027b(); return; };                     //
}
...
void sub_0661()
{
    ...
    _out(_dx, _ax);                             //out dx, ax
    _di = memory16(_ds, _bp + 0);               //mov di, word ptr [bp+0x0]
...
void sub_1bae()
{
    ...
    _ASSERT(0 /* check carry */); _rcr(_ax, 1); //rcr ax, 1
    _rcr(_ax, 1);                               //rcr ax, 1
...
void sub_1c15()
{
    ...
    _ax = 0x0f02;                               //mov ax, 0xf02
    _out(_dx, _ax);                             //out dx, ax
    _ah = memory(_es, _di);                     //mov ah, byte ptr es:[di]
    _ah = 0x01;                                 //mov ah, 0x1
    _out(_dx, _ax);                             //out dx, ax
    _lodsw<MemData, DirForward>();              //lodsw
...
loc_1d09:                                       //loc_1d09:
    ...
    if (memory16(_ds, 38700) != 0)              //jnz sub_1d30
        { sub_1d30(); return; }
    sub_1cb3();                                 //call sub_1cb3
    _STOP_("sp-trace-fail");                    //sub_1ce7 endp_failed
    _STOP_("continues");                        //sub_1d30 proc near
}
...
loc_1dea:                                       //loc_1dea:
    _lodsb<MemData, DirForward>();              //lodsb
    memory(_ds, _di) = _al;                     //mov byte ptr [di], al
    memory(_ds, _di + 320) = _al;               //mov byte ptr [di+0x140], al
    memory(_ds, _di + 640) = _al;               //mov byte ptr [di+0x280], al
    memory(_ds, _di + 960) = _al;               //mov byte ptr [di+0x3c0], al
    memory(_ds, _di + 1280) = _al;              //mov byte ptr [di+0x500], al
    memory(_ds, _di + 1600) = _al;              //mov byte ptr [di+0x640], al
    memory(_ds, _di + 1920) = _al;              //mov byte ptr [di+0x780], al
    memory(_ds, _di + 2240) = _al;              //mov byte ptr [di+0x8c0], al
    _di += _dx;                                 //add di, dx
...
void sub_2263()
{
    ...
    if (_al != 0x47)                            //jnz loc_2276
        goto loc_2276;
    _STOP_("goto loc_2501");                    //jmp loc_2501
loc_2276:                                       //loc_2276:
...
void sub_2861()
{
    ...
    sub_12ae();                                 //call sub_12ae
    if (_FIXME_)                                //jnz loc_286f
        goto loc_286f;
    sub_19cf();                                 //call sub_19cf
loc_286f:                                       //loc_286f:
    sub_13ac();                                 //call sub_13ac
    if (_FIXME_)                                //jnz loc_2877
        goto loc_2877;
    _STOP_("goto loc_24c4");                    //jmp loc_24c4
loc_2877:                                       //loc_2877:
    _al = memory(_ds, 0x7eae);                  //mov al, [0x7eae]
