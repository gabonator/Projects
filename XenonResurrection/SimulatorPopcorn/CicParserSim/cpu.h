...
#define _tx _reg.t.r16
#define _th _reg.t.r8.h
#define _tl _reg.t.r8.l
...
void _WARN_(const char* msg) {std::cout << "Skipping !!!! " << msg << "\n"; }
...
void memoryVideoXor(WORD seg, WORD ofs, WORD x)
{
    int y = memoryVideoGet(seg, ofs);
    y ^= x;
    memoryVideoSet(seg, ofs, y);
}

void memoryVideoShl(WORD seg, WORD ofs, int x)
{
    _ASSERT(x == 1);
    int y = memoryVideoGet(seg, ofs);
    _flags.carry = !!(y & 0x80);
...
void memoryVideoXor16(WORD seg, WORD ofs, WORD x)
{
    memoryVideoXor(seg, ofs, x&0xff);
    memoryVideoXor(seg, ofs+1, x>>8);
}

void _mul(const WORD& w)
{
    int v = w * _ax;
    _ax = v & 0xffff;
    _dx = v >> 16;
}
void _mul(const BYTE& b)
{
    int v = b * _al; /// !!!!!!!!!!! AAAAAH!!!!!!! 
    _ax = v & 0xffff;
}

void _imul(const WORD& w)
{
