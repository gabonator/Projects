...
constexpr WORD _seg000 = 0x1000; // stack
constexpr WORD _seg001 = 0x1020; // data
constexpr WORD _seg002 = 0x13EC; // program

CEga mVideo;
reg_t _reg;
uint8_t data[0x10000*4];
CSdl mSdl;
...
{
    char fname[128];
    sprintf(fname, "/Users/gabrielvalky/Temp/%s", suffix);
    FILE* f = fopen(fname, "rb");
    fread(buffer + _seg000*16, len, 1, f);
    fclose(f);
}
...
int main(int argc, const char * argv[]) {
    ...
    loadSegment(data, "CD-MAN.data", 0x3ec0);
    mSdl.Init();
    start();
    mSdl.Deinit();
    return 0;
