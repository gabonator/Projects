...
constexpr WORD _seg001 = 0x1010;
constexpr WORD _seg002 = 0x1723;
...
//int _p42 = 0, _buffer42 = 0, _buffer = 0;
...
// aj 0x6bc sa analyzuje
    ...
    for (int i=0; keys[i]; i++)
        if (k == keys[i])
            memory(_seg001, 0x6B8+i) = p ? 0x00 : 0x80;

    if (p)
        memory16(_seg001, 0x693)++;
}
...
int main(int argc, const char * argv[]) {
    ...
    loadSegment(data+_seg001*16, "datasegment", 28976); //33436);
    mSdl.Init();
    while (1)
    {
        start();
