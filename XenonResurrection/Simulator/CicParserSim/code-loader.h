void sub_15C8B()
{
    std::cout << "sub_15C8B - load all files\n";
    // x2spr.dat
...
void sub_10DA3() // fix bitmap?
{
  ...
  _bp <<= 1;                                      //shl bp, 1
  _bp <<= 1;                                      //shl bp, 1
  //_STOP_("call cs:off_10DB1[bp]");                //call cs:off_10DB1[bp]
    /*
...
void sub_1011C() // otvori a nacita cely subor
{
  ...
  if (!_flags.carry)                                    //jnb short loc_10126
    goto loc_10126;
    _ASSERT(0);
  //goto loc_15E04;                                 //jmp loc_15E04 TODO: exit app
loc_10126:                                        //loc_10126:
  _push(_ds);                                     //push ds
  ...
  _interrupt(0x21);                               //int 21h
  if (/*podarilo sa nacitat*/1)                                    //jnb short loc_1014C
    goto loc_1014C;
  //goto loc_15E04;                                 //jmp loc_15E04 // TODO: exit app
loc_1014C:                                        //loc_1014C:
  _ax = memory16(_ds, 0x2177);                    //mov ax, word_277C7  file[b]
  memory16(_ds, 0x0) = _ax;                       //mov word_25650, ax
  _ax = memory16(_ds, 0x2179);                    //mov ax, word_277C9  file[d]
  memory16(_ds, 0x2) = _ax;                       //mov word_25652, ax
  _al = memory(_ds, 0x216C);                      //mov al, byte_277BC file[0]
  _ah = _ah ^ _ah;                                //xor ah, ah   --- -ds:216C je asi pocet obrazkov
  _ax += 0x216e;                                  //add ax, 216Eh
  memory16(_ds, 0x216A) = _ax;                    //mov word_277BA, ax
  sub_1019A();                                    //call sub_1019A --  problem
  _ah = 0x3e;                                     //mov ah, 3Eh
  _bx = memory16(_ds, 0x2370);                    //mov bx, word_279C0
  _interrupt(0x21);                               //int 21h
  _ax = _pop();                                   //pop ax
...
loc_101A9:                                        //loc_101A9:
  sub_10230();                                    //call sub_10230 -- pripravi nejake tabulky
  sub_102A8();                                    //call sub_102A8 -- pripravi tabulku 000...rise..fall
  _di = 0x0fc4;                                   //mov di, 0FC4h
loc_101B2:                                        //loc_101B2:
  _push(_di);                                     //push di
  sub_102D4();                                    //call sub_102D4 --- tuto citame data!!!
  _di = _ax;                                      //mov di, ax
  ...
  _push(_es);                                     //push es
  _push(_di);                                     //push di
  _les(_di, 0x236c); //_les(_di, _FIXME_);                             //les di, dword_2A89C
  ...
  _di &= 0x0fff;                                  //and di, 0FFFh
    _flags.carry = memory16(_ds, 0x0) == 0;
  memory16(_ds, 0x0) -= 1;                        //sub word_28530, 1
  memory16(_ds, 0x2) -= _flags.carry;
    
  //_sbb(memory16(_ds, 0x2), 0);                    //sbb word_28532, 0
...
loc_101E0:                                        //loc_101E0:
  ...
  _cx -= 0xfd;                                    //sub cx, 0FDh
    _flags.carry = memory16(_ds, 0x0) == _cx; // TODO:!!!!!!!!!!!!
  memory16(_ds, 0x0) -= _cx;                      //sub word_28530, cx
//    _ASSERT(0);
    memory16(_ds, 0x2) -= _flags.carry;
...
#warning neviem!
    _es = _ds;
    _les(_di, 0x236c); //0x2a89c-0x28530
  //_les(_di, _FIXME_);                             //les di, dword_2A89C
...
void sub_104C9() // -- read data 2565:216c
{
  _push(_si);                                     //push si
  ...
  if ((short)_si < (short)0x236c)                 //jl short loc_104DA
    goto loc_104DA;
    std::cout << "R3";
  sub_10174();                                    //call sub_10174 -- read data
...
loc_10312:                                        //loc_10312:
  ...
  if ((short)_si < (short)0x236c)                 //jl short loc_10323
    goto loc_10323; // ked je SI vela, nacitaj znova
    std::cout << "R1";
...
loc_10323:                                        //loc_10323:
  ...
  if ((short)_si < (short)0x236c)                 //jl short loc_1033E
    goto loc_1033E;  // ked je SI vela, nacitaj znova
    std::cout << "R2";
  sub_10174();                                     //call sub_10174
  _si = 0x216c;                                   //mov si, 216Ch
loc_1033E:                                        //loc_1033E:
...
void sub_10174() // seg003  2565:216c
{
  _push(_di);                                     //push di
  _push(_bx);                                     //push bx
  _push(_cx);                                     //push cx
  ...
  _rep_stosw<MemAuto, DirAuto>();                 //rep stosw -- memclr
