...
loc_1F708:                                        //loc_1F708:
    ...
    _ror(_ax, _cl);                               //ror ax, cl
    memory(_es, _di + 4) &= _ah;                  //and es:[di+4], ah
    _di += 0x28;                                  //add di, 28h
    _bl -= 1;                                     //dec bl
...
loc_14717:                                        //loc_14717:
    ...
    _ASSERT(0);
    //if ((type)memory16(_ds, 0x912E) > 0 /*CHECK*/)//jg short locret_1472C
    //  goto locret_1472C;
    _ax = memory16(_ds, 0x9130);                  //mov ax, word_31660
...
void sub_11D1E()
{
    ...
    _cx -= 0x10;                                  //sub cx, 10h
    _ASSERT(0);
    //if ((type)_cx <= (type) /*CHECK*/)            //jle short loc_11D97
//      goto loc_11D97;
loc_11D71:                                        //loc_11D71:
    ...
    _cx -= 0x10;                                  //sub cx, 10h
    _ASSERT(0);
//    if ((type)_cx > 0 /*CHECK*/)                  //jg short loc_11D71
//      goto loc_11D71;
loc_11D97:                                        //loc_11D97:
    _si = _pop();                                 //pop si
}
...
loc_2066C:                                        //loc_2066C:
    _lodsw<MemAuto, DirAuto>();                   //lodsw
    _ASSERT(0);
    //if ((SIGNED?)_ax < 0)                         //js short loc_206EA
      //goto loc_206EA;
    _bx = _ax;                                    //mov bx, ax
    _lodsw<MemAuto, DirAuto>();                   //lodsw
    _dx = _ax;                                    //mov dx, ax
...
loc_13620:                                        //loc_13620:
    _ax = memory16(_ds, _si);                     //mov ax, [si]
    _ASSERT(0);
    //if ((SIGNED?)_ax < 0)                         //js short loc_13643
      //goto loc_13643;
...
loc_13656:                                        //loc_13656:
    _ax = memory16(_ds, _si);                     //mov ax, [si]
    _ASSERT(0);
    //if ((SIGNED?)_ax < 0)                         //js short loc_13679
      //goto loc_13679;
...
loc_13360:    _ASSERT(0); return;
}

void sub_10FB4()
{
    _ASSERT(0); WORD _cs = seg001;
    memory16(_ds, 0x8E74) = _bx;                  //mov word_313A4, bx
    _bx = memory16(_ds, 0x991C);                  //mov bx, word_31E4C
    _bx <<= 1;                                    //shl bx, 1
...
loc_135CE: _ASSERT(0); return;
loc_16F7F: _ASSERT(0); return;
loc_16F00: _ASSERT(0); return;
loc_16FE0: _ASSERT(0); return;
loc_1705B: _ASSERT(0); return;
}
...
loc_1EBF2:                                        //loc_1EBF2:
    ...
    _cx = _ax;                                    //mov cx, ax
    _ASSERT(0);
    //if ((SIGNED?)_dx >= 0)                        //jns short loc_1EC21
     // goto loc_1EC21;
    _push(_di);                                   //push di
    _push(_cx);                                   //push cx
loc_1EC0A:                                        //loc_1EC0A:
...
loc_1ED43:                                        //loc_1ED43:
    ...
    _ax = 0xa400;                                 //mov ax, 0A400h
    _ds = _ax;                                    //mov ds, ax
    memory(_ss, 280496) = 0;                      //mov ss:byte_447B0, 0
    memory(_ss, 280497) = 1;                      //mov ss:byte_447B0+1, 1
loc_1ED75:                                        //loc_1ED75:
    _dx = 0x03c4;                                 //mov dx, 3C4h
    _ah = memory(_ss, 280497);                    //mov ah, ss:byte_447B0+1
    _al = 0x02;                                   //mov al, 2
    _out(_dx, _ax);                               //out dx, ax
    _dx = 0x03ce;                                 //mov dx, 3CEh
    _ah = memory(_ss, 280496);                    //mov ah, ss:byte_447B0
    _al = 0x04;                                   //mov al, 4
    _out(_dx, _ax);                               //out dx, ax
    _push(_si);                                   //push si
    _push(_di);                                   //push di
...
loc_1ED8E:                                        //loc_1ED8E:
    ...
    _di = _pop();                                 //pop di
    _si = _pop();                                 //pop si
    memory(_ss, 280496) += 1;                     //inc ss:byte_447B0
    memory(_ss, 280497) <<= 1;                    //shl ss:byte_447B0+1, 1
    if ((char)memory(_ss, 280496) < (char)4)      //jl short loc_1ED75
      goto loc_1ED75;
    _dx = 0x03c4;                                 //mov dx, 3C4h
    _ax = 0x0f02;                                 //mov ax, 0F02h
    _out(_dx, _ax);                               //out dx, ax
...
loc_14A36:                                        //loc_14A36:
    ...
    _ASSERT(0);
//    if ((type)memory16(_ds, 0xFCFE) <= (type) /*CHECK*/)
//      goto loc_14A48;
    goto loc_1499F;                               //jmp loc_1499F
loc_14A48:                                        //loc_14A48:
    _ax = memory16(_ds, 0xFCFE);                  //mov ax, word_3822E
...
loc_14A4F:                                        //loc_14A4F:
    ...
    _ASSERT(0);
//    if ((SIGNED?)_dx < 0)                         //js short loc_14A4F
//      goto loc_14A4F;
    _di = memory16(_ds, _si + 56);                //mov di, [si+38h]
    _di += _dx;                                   //add di, dx
    memory16(_ds, _si + 58) = _di;                //mov [si+3Ah], di
...
{
    _ASSERT(0);
//    if ((SIGNED?)_ax >= 0)                        //jns short sub_11BB4
//      goto sub_11BB4;
    _ax = -_ax;                                   //neg ax
...
{
    _ASSERT(0);
//    if ((SIGNED?)_bx >= 0)                        //jns short sub_11BA2
//      goto sub_11BA2;
    _bx = -_bx;                                   //neg bx
