...
constexpr WORD _dseg = 0x186a;
constexpr WORD _seg002 = 0x1922;
constexpr WORD _segargs = 0x2000;
...
//    }
    ...
    if (segment == _ss && offset == 0x48+8)
    {
//        for (int i=0; i<32; i++)
//            std::cout << (char)datasegment[(segment-0x1000)*16+offset-32+i];
        int f = 9;
    }
...
{
    if (segment != _dseg && segment != _seg000 && segment != _segargs && segment != 0x109f)
    {
        _ASSERT(0);
    }
    segment -= 0x1000;
...
void loadSegment(uint8_t* buffer, const char* suffix, int len)
{
    ...
    char fname[128];
    sprintf(fname, "/Users/gabrielvalky/Documents/git/Projects/XenonResurrection/InputAv/%s", suffix);
...
int main(int argc, const char * argv[]) {
    ...
    loadSegment(datasegment, "av.data", 36870); //33436);
    mSdl.Init();
    while (1)
    {
        start();
