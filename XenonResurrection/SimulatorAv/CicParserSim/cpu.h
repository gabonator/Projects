...
{
    _sp += n;
    _ASSERT( _sp >= 0 && _sp < 0xa0);
}

void StackWrite(int nOffset, WORD nData)
{
    _ASSERT(nOffset >= 0 && nOffset < 0xa0 && (nOffset&1) == 0);
    memory16(_ss, nOffset) = nData;
}

WORD StackRead(int nOffset)
{
    _ASSERT(nOffset >= 0 && nOffset < 0xa0 && (nOffset&1) == 0);
    return memory16(_ss, nOffset);
}
...
//    _es = seg;
//    reg = addr;
    reg = memory16(seg, addr);
    _es = memory16(seg, addr+2);
}
void _lds(WORD& reg, WORD seg, WORD addr)
{
//    _es = seg;
//    reg = addr;
    reg = memory16(seg, addr);
    _ds = memory16(seg, addr+2);
}
...
void _repne_scasb()
{
    ...
    while ( _cx-- && _flags.zero == 0 )
    {
        ...
        _flags.carry = _al < memory(_es, _di);
        _di++;
    }
}
...
WORD& memory16(_ss, int a, int b)
{
    return memory16(_ss, a+b);
}
*/
