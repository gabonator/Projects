...
loc_18498:                                      //loc_18498:
    ...
    _ax = 0xf000;                               //mov ax, 0F000h
    _es = _ax;                                  //mov es, ax
    if (memory(_es, 0xFFFE) == 0xfc)            //jz short loc_184C4
        goto loc_184C4;
...
loc_1883A:                                      //loc_1883A:
    ...
    _out(_dx, _al);                             //out dx, al
    _dx = _si;                                  //mov dx, si
    _al = memory(_es, _bp + 0);                 //mov al, es:[bp+0]
    _al = memory(_cs, _bx + _di);               //mov al, cs:[bx+di]
    memory(_es, _bp + 0) = _al;                 //mov es:[bp+0], al
    _ch >>= 1;                                  //shr ch, 1
    _ah -= 1;                                   //dec ah
...
void sub_188B9()
{
    if (memory(_ds, 0x6471) == 0x01)            //jz short locret_188CC
        goto locret_188CC;
    _push(_dx);                                 //push dx
    _push(_ax);                                 //push ax
...
void sub_188E1()
{
    if (memory(_ds, 0x6471) == 0x01)            //jz short locret_188F4
        goto locret_188F4;
    _push(_dx);                                 //push dx
    _push(_ax);                                 //push ax
    _dx = 0x03ce;                               //mov dx, 3CEh
...
loc_18A1D:                                      //loc_18A1D:
    ...
    if (_ax != 0)                               //jnz short loc_18A1B
        goto loc_18A1B;
    _ax = memory16(_es, 0x6C);                  //mov ax, es:6Ch
    _ax -= _bx;                                 //sub ax, bx
...
void sub_18B77()
{
    ...
    _dx = _dx + 0x8249 + _flags.carry; _ASSERT(0);//adc dx, 8249h
...
void sub_19C43()
{
    ...
    _ax = 0x0040;                               //mov ax, 40h
    _es = _ax;                                  //mov es, ax
    if (memory(_es, 0x17) & 0x20)               //jnz short loc_19C57
        goto loc_19C57;
    memory(_es, 0x17) |= 0x20;                  //or byte ptr es:17h, 20h
loc_19C57:                                      //loc_19C57:
    _es = _pop();                               //pop es
}
...
loc_19E24:                                      //loc_19E24:
    ...
    if (_FIXME_)                                //jnb short loc_19E3F
        goto loc_19E3F;
loc_19E29:                                      //loc_19E29:
    _al = _ah;                                  //mov al, ah
    ...
    _di <<= 1;                                  //shl di, 1
    {_indirectCall(memory16(_ds, _di + 23778)); return; }
loc_19E3F:                                      //loc_19E3F:
...
loc_1A454:                                      //loc_1A454:
    ...
    memory(_es, _di) = 0xff;                    //mov byte ptr es:[di], 0FFh
    _di += 1;                                   //inc di
...
loc_1A51E:                                      //loc_1A51E:
    ...
    memory16(_es, _di + 4) = _bx;               //mov es:[di+4], bx
    _bl += 1;                                   //inc bl
...
loc_1A53D:                                      //loc_1A53D:
    memory16(_es, _di) = 0x3020;                //mov word ptr es:[di], 3020h
    memory(_es, _di + 0x2) = 0x3032;            //mov word ptr es:[di+2], 3032h
    memory(_es, _di + 0x4) = 0x3030;            //mov word ptr es:[di+4], 3030h
...
void sub_1A699()
{
    ...
    _ax = 0x0040;                               //mov ax, 40h
    _es = _ax;                                  //mov es, ax
    _bl = memory(_es, 0x87);                    //mov bl, es:87h
    _bl |= _bl;                                 //or bl, bl
...
void sub_1AB24()
{
    ...
    _cl = 0x03;                                 //mov cl, 3
    _ASSERT(0 /* check carry */); _rcr(_dx, _cl);//rcr dx, cl
...
{
    _dx = memory16(_ds, 0x4289);                //mov dx, ds:4289h
    _dx = _dx + 0x9428 + _flags.carry; _ASSERT(0);//adc dx, 9428h
    _cl = 0x03;                                 //mov cl, 3
    _ASSERT(0 /* check carry */); _rcr(_dx, _cl);//rcr dx, cl
