...
void UpdateKeys(cicoKeys_t& k);
void KeySync()
{
        uint8_t v = 0;
        if (pathReading)
        {
          if (fread(&v, 1, 1, path)==0)
          {
            pathReading = false;
...
//          printf("Path: %x\n", v);
        ...
        //printf("getkey: %d %x %x\n", pathReading, ftell(path), cicoKeys.toByte());
...
void UpdateKeys(cicoKeys_t& k)
{
//  memoryASet(0x30d4, 0xc643, k.space);
  memoryASet(0x30d4, 0xa545, k.enter ? 0x1c : 0);
  memoryASet(0x30d4, 0xc647+0x4b*2, k.left);
  memoryASet(0x30d4, 0xc647+0x4d*2, k.right);
  memoryASet(0x30d4, 0xc647+0x48*2, k.up);
  memoryASet(0x30d4, 0xc647+0x50*2, k.down);
  memoryASet(0x30d4, 0xc647+0x1d*2, k.ctrl);
  memoryASet(0x30d4, 0xc647+0x38*2, k.alt);
  memoryASet(0x30d4, 0xc647+0x39*2, k.space);
//  memoryASet(0x30d4, 0xc647+0x39*2, k.alt);
//  memoryASet(0x30d4, 0xc647+0x39*2, k.alt);
}
...
void onKey(int k, int p)
{
  switch (k)
  {
    case SDL_SCANCODE_ESCAPE: exit(1);
    case SDL_SCANCODE_SPACE: cicoKeys.space = p; break;
    case SDL_SCANCODE_RETURN: cicoKeys.enter = p; break;
    case SDL_SCANCODE_LCTRL: cicoKeys.ctrl = p; break;
    case SDL_SCANCODE_LEFT: cicoKeys.left = p; break;
    case SDL_SCANCODE_RIGHT: cicoKeys.right = p; break;
    case SDL_SCANCODE_UP: cicoKeys.up = p; break;
    case SDL_SCANCODE_DOWN: cicoKeys.down = p; break;
    case SDL_SCANCODE_LALT: cicoKeys.alt = p; break;
  }
