class Ega
{
  ...
  interrupt(i)
  {
    switch (r8[ah])
    {
      case 0x00:
        EGA.cfgDataRotate = 0;
        for (let i=0; i<16; i++) EGA.palette[i] = EGA.defPalette[i];
...
return true
    ...
    PortWrite8: function(port, val)
    {
        ...
        if (port == 0x3c9)
        {
            ...
            let ind = Math.floor(EGA._palIndex/3);
            let ch = 2-(EGA._palIndex%3);
            //if (val == 255)
            //  return true; // ?
            let mask = 0xff << (ch*8)
            let or = (val * 4) << (ch*8);
            //console.log("pal write", ind, val*4);
            this.palette[ind] &= ~mask;
            this.palette[ind] |= or; 
            EGA._palIndex++;
/*
...
//            EGA.PortWrite16(0x3c4, (val << 8)|EGA._last3c4);
        ...
        if (port == 0x3c7)
        {
            this.vgareadcolorindex = val*3; //Math.floor(val*3/2); //*3/2;
            return true;
        }
    ...
    {
        let r = ((rgb & 4) ? 2 : 0) + ((rgb & 32) ? 1 : 0);
        let g = ((rgb & 2) ? 2 : 0) + ((rgb & 16) ? 1 : 0);
        let b = ((rgb & 1) ? 2 : 0) + ((rgb & 8) ? 1 : 0);
        r = r * 255 / 3;
        g = g * 255 / 3;
        b = b * 255 / 3;
        this.palette[index] = b | (g << 8) | (r << 16) | 0x40000000;
    ...
    PortRead8: function(port)
    {
      if (port == 0x3c9)
      {
            let cindex = Math.floor(this.vgareadcolorindex/3);
            let cch = 2-(vgareadcolorindex%3);
            let data = (this.palette[cindex] >> (cch*8)) & 255;
//            printf("get pal %d %d %d = %d\n", vgareadcolorindex, cindex, 2-cch, data);
            this.vgareadcolorindex++;
            return data;
      }
      if (EGA._crtReg == 0x0c)
