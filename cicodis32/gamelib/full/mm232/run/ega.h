...
{
public:
    uint8_t buffer[320*240*4]; // mm2 =320*240*4
    uint8_t palette[0x300];
    int readcolorindex{0};

    int planemask = 0;
    bool usePlanes = true; // worms = false, mm2 = true
    int pitch = 320;
    int mPanning{0};
    ...
    virtual bool PortWrite16(int port, int data) override
    {
//        if (data % 0x100 != 0x2 && data % 0x100 != 0x4)
//        {
//            int f = 9;
//        }
//        if (port == 0x3c0)
...
//        }
    ...
    virtual bool PortWrite8(int port, int data) override
    {
        ...
        if (port == 0x3cf)
        {
            // planar http://www.osdever.net/FreeVGA/vga/graphreg.htm
            assert(data == 1 || data == 0);
            planarWriteMode = data&3;
        }
        if (port == 0x3ce || port == 0x3cf || port == 0x3d4 || port == 0x3d5 || port == 0x3c4 || port == 0x3c5 || port == 0x3c2)
        {
    ...
    virtual void Write(uint32_t dwAddr, uint8_t bWrite) override
    {
        ...
        int addr = (dwAddr - 0xa000*16)*4;
        assert(addr >= 0 && addr < sizeof(buffer));
        if (0 && planarWriteMode==1)
        {
            if (planemask & 1)
                buffer[addr] = buffer[latchAddr];
            if (planemask & 2)
                buffer[addr+1] = buffer[latchAddr+1];
            if (planemask & 4)
                buffer[addr+2] = buffer[latchAddr+2];
            if (planemask & 8)
                buffer[addr+3] = buffer[latchAddr+3];
            return;
//            bWrite = 0xff;
        }
        if (planemask & 1)
            buffer[addr] = bWrite;
        if (planemask & 2)
            buffer[addr+1] = bWrite;
        if (planemask & 4)
            buffer[addr+2] = bWrite;
        if (planemask & 8)
            buffer[addr+3] = bWrite;
    }
    virtual uint8_t Read(uint32_t dwAddr) override
    {
        if (!usePlanes)
...
//        return buffer[addr];
        if (planemask & 1)
            return buffer[addr];
        if (planemask & 2)
            return buffer[addr+1];
        if (planemask & 4)
            return buffer[addr+2];
        if (planemask & 8)
            return buffer[addr+3];
        assert(0);
        return 0;
    }
    virtual uint32_t GetPixel(int x, int y) override
    {
        ...
        x += mPanning; //(ReadMemory(ds, 0x1f1c38) & 15); // mPanning;
        if (0)
        {
            int base = 0;// 80*(x%4); //0; //(x%4)*80*240;
            x /= 4;
            x %= 80;
            
            uint8_t c = buffer[base+y*80*4+x];
            return (palette[3*c]*4) | ((palette[3*c+1]*4)<<8) | ((palette[3*c+2]*4)<<16);
        }
        
        //static int pitch = 336; //256*10;
//        if (x==0 && y == 0)
//        {
//            pitch++;
//            if (pitch/10 > 1000)
