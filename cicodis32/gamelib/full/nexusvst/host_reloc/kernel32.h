...
#elif defined(WASM)
static const char* VFS_SANDBOX = "/nexus/";
#else
...
//        name.find(".nsk") != std::string::npos) {
    ...
    if (slot < 0 || slot >= 64) return 0xffffffff;

#ifdef WASM
    std::string vfsPath = vfsTranslate(name);
    if (vfsPath.empty()) {
        printf("CreateFileA FAIL (sandbox): %s\n", name.c_str());
        return 0xffffffff;
    }
    ...
    fileSlots[slot].size = (uint32_t)result;
    printf("CreateFileA: '%s' -> handle 0x%x (%u bytes)\n", name.c_str(), handle, (uint32_t)result);
    return handle;
#else
    ...
    if (!fp) { std::string vfsPath = vfsTranslate(name); fp = fopen(vfsPath.c_str(), "rb"); }
    if (!fp) { fp = fopen(name.c_str(), "rb"); }
    if (!fp) {
        printf("CreateFileA FAIL: %s\n", name.c_str());
        return 0xffffffff;
    }
    fseek(fp, 0, SEEK_END);
...
uint32_t GetModuleFileNameA() {
    ...
    for (i = 0; i < maxLen - 1; i++) {
        uint8_t c = bla[i]; //memoryAGet(ds, src + i);
...
uint32_t LoadLibraryA() {
    ...
    if (name.find("SYNSOEMU") != std::string::npos)
        return 0x01000000; // SYNSOEMU base address in our memory layout
    return 0x00000004; // fake module handle for other DLLs
}
uint32_t GetProcAddress() {
    ...
    if (namePtr < 0x10000) {
        ...
        static const uint32_t synsoOrdinals[29] = {
            0,           // ordinal 0: unused
            0x10018fa,  // ord 1
            0x10018fa,  // ord 2 (posInstantiateObject thunk)
            0x1001582,  // ord 3 (posInstantiateObject)
            0x10015fc,  // ord 4 (posCallMethod)
            0x10015ed,  // ord 5
            0x10015a2,  // ord 6 (posDeleteObject)
            0x10018fa,  // ord 7
            0x10018fa,  // ord 8
            0x10018fa,  // ord 9
            0x10018fa,  // ord 10
            ...
            0x10018fa,  // ord 15
            0x10015de,  // ord 16
            0x1001531,  // ord 17 (posRegisterApplicationWithClasses)
            0x10015b7,  // ord 18 (posGetInfoWithCommand)
            0x10018fa,  // ord 19
            0x10018fa,  // ord 20
            0x10018fa,  // ord 21
            0x10018fa,  // ord 22
            ...
            0x10018fa,  // ord 24
            0x10018fa,  // ord 25
            0x10018bb,  // ord 26 (posDCPCreateEncryptionContext)
            0x10018e5,  // ord 27 (posDCPEncryptData)
            0x10018d0,  // ord 28
        };
...
uint32_t GetFileAttributesA() {
    ...
    for (auto& c : name) if (c == '\\') c = '/';
#ifdef WASM
    // In WASM assume files exist; CreateFileA will fail if they don't
...
uint32_t FindFirstFileA() {
...
#ifdef WASM
    {
        ...
        // Normalize in C++: backslash→slash, lowercase, strip trailing /*
        for (auto& c : pattern) if (c == '\\') c = '/';
        if (pattern.size() >= 2 && pattern.back() == '*') {
            auto sl = pattern.rfind('/', pattern.size() - 2);
            if (sl != std::string::npos) pattern = pattern.substr(0, sl);
        }
