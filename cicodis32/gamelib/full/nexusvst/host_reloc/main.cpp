...
//const char* appRootPath = "res_reloc/";
const char* appRootPath = "/Users/gabrielvalky/Documents/git/Projects/cicodis32/gamelib/full/nexusvst/res_reloc/";
...
void sub_77778888()
{
                    ...
                    switch (_amc_op) {
                        ...
                        case 7: { // audioMasterGetTime
                            ...
                            // 0x593000: samplePos (double), 0x593008: ppqPos (double) [reloc: MEM_BASE=0]
                            double _samplePos, _ppqPos;
                            { uint64_t t; t=memoryAGet64(ds,0x593000); memcpy(&_samplePos,&t,8); t=memoryAGet64(ds,0x593008); memcpy(&_ppqPos,&t,8); }
                            ...
                            uint32_t _transportFlag = memoryAGet32(ds, 0x593010);
...
void start();
void VSTPluginMain();
void sub_1d100();  // VSTPluginMain entry
void sub_15070();  // real plugin init (called by sub_1001d100)
void sub_1b3a0();  // dispatcher
void sub_31d10();  // bus struct initializer
void sub_1b3a0(); // dispatcher
void sub_1b470(); // process replacing
void _initterm();
...
void print_alloc_stats(); // defined in crt_all.cpp (included via nexus.cpp)
...
int main(int argc, char* argv[]) {
    ...
    loadOverlay("SYNSOEMU_rdata.bin", 0x1007000);
    loadOverlay("SYNSOEMU_data.bin", 0x10a6000);
    ...
    for (int i = 0; contentPath[i]; i++)
        memoryASet(0, 0x57f420 + i, contentPath[i]);
    memoryASet(0, 0x57f420 + strlen(contentPath), 0);
    ...
    assert(memoryAGet32(0, pluginPtr + 4) == 0x1b3a0);
...
//            sendMidi(0x80, midiNote+12, 0);
        ...
        push32(pluginPtr);
        ecx = thisPtr;
        sub_1b470(); // processReplacingAddr
