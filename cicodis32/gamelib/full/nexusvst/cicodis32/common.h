...
//    };
    ...
    bool quiet{false};
    bool stackGuard{false};
    bool verboseIndirectCall{false};

    std::vector<address_t> procList;
    std::set<address_t> blackList;  // addresses to skip during recursive disassembly
...
//        return nullptr;
    ...
    bool preprocess{true};  // SEH removal pass (set to false to emit raw SEH code)
