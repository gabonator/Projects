...
std::vector<address_t> BuildEntriesList(const shared<Loader> &loader, const std::shared_ptr<Options> &options) {
    ...
    for (shared<jumpTable_t> j : options->jumpTables)
        if (j->type == jumpTable_t::Call || j->type == jumpTable_t::CallWords || j->type == jumpTable_t::CallDwords || j->type == jumpTable_t::Call32)
...
//        else
    ...
    for (address_t proc : allMethods)
    {
        ...
        // --- SEH removal pass ---
        // Detects and removes SEH prologue/epilogue patterns from the IR.
        // SEH prologue: push32(-1), push32(handler_addr), eax=memoryAGet(fs,0), push32(eax), memoryASet(fs,0,esp)
        // SEH epilogue: reg=memoryAGet(ss,[ebp-N]), memoryASet(fs,0,reg)
        // After removal, adjusts esp cleanup values so the stack stays balanced.
            ...
            if (argc > 2)
            {
                for (int i=2; i<argc; i++)
                {
                    ...
                    if (address_t::mode32)
                        ...
                        options->procList.push_back(address_t::fromString(argv[i]));
                }
                if (!DoIteration(loader, options, analyser))
                    return 1;
            }
