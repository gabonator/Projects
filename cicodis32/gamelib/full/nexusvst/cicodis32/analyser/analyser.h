...
struct funcInfo_t {
    ...
    std::string makeProcIdentifier() const
    {
        ...
        if (temp & (int)procRequest_t::returnSign)
        {
            extraInfo += " +returnSign";
            temp ^= (int)procRequest_t::returnSign;
        }
...
public:
    ...
    {
        if (mOptions->blackList.find(method) != mOptions->blackList.end())
            return;
            ...
            for (address_t methodToProcess : methodsToProcess)
            {
                ...
                for (const auto& [newCallTarget, newCallConv] : GetCalls(methodToProcess))
                {
                    ...
                    if (methodsProcessed.find(newCallTarget) == methodsProcessed.end() &&
                        mOptions->blackList.find(newCallTarget) == mOptions->blackList.end() &&
...
//        { fprintf(stderr, "analyser assert skipped at line %d\n", __LINE__); }
    ...
    std::vector<std::pair<address_t, procRequest_t>> GetCalls(address_t proc)
    {
        ...
        if (code.size() == 1 && code.begin()->second && code.begin()->second->instr && code.begin()->second->instr->mId == X86_INS_JMP)
        {
            // stub
        ...
        {
            if (!p.second || !p.second->instr) continue;
