...
public:
    ...
    virtual std::set<address_t> AnalyseInstruction(shared<instrInfo_t> info, shared<info_t> info_) override
    {
        ...
        static const uint64_t readFlagMask[128] =
        {
            ...
            ['p'] = X86_EFLAGS_PRIOR_PF | X86_EFLAGS_TEST_PF,
        };
        ...
        static const uint64_t applyFlagMask[128] =
        {
            ...
            ['p'] = X86_EFLAGS_MODIFY_PF | X86_EFLAGS_RESET_PF | X86_EFLAGS_SET_PF,
        };
        ...
        for (instrInfo_t::instrInfoFlag_t* flag : info->Flags())
        {
            ...
            if (info->instr->mTemplate.ret && info_->func.request != procRequest_t::none)
            {
                ...
                if (flag->type == 'z' && ((int)info_->func.request & (int)procRequest_t::returnZero))
                    flag->needed = true;
                if (flag->type == 's' && ((int)info_->func.request & (int)procRequest_t::returnSign))
                    flag->needed = true;
            }
    ...
    void applyFlags(shared<CapInstr> instr, uint64_t modifyMask, instrInfo_t::instrInfoFlag_t* flag)
    {
        ...
        if (instr->mId == X86_INS_SAHF || instr->mId == X86_INS_INT || instr->mId == X86_INS_CALL || instr->mId == X86_INS_LCALL || (instr->mDetail.eflags & modifyMask))
        {
            ...
            if (flag->type == 'z' && instr->mTemplate.savedVisiblyZero) // TODO
                flag->visible = true;
            if (flag->type == 'p' && instr->mTemplate.savedVisiblyParity)
                flag->visible = true;
        }
    };
