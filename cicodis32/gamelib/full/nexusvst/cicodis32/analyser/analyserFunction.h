...
public:
    ...
    virtual void AnalyseProc(address_t proc, procRequest_t req, int stackDrop)
    {
        ...
        while (!head.empty())
        {
            ...
            for (auto link : head)
            {
                ...
                auto codeIt = code.find(link.second);
                if (codeIt == code.end() || !codeIt->second || !codeIt->second->instr)
                    continue;
                auto instr = codeIt->second->instr;
    ...
    void DumpCode(address_t proc, code_t& code, procRequest_t req, int stackDrop)
    {
        ...
        if ((int)req & (int)procRequest_t::returnZero)
            printf(" +zero");
        if ((int)req & (int)procRequest_t::returnSign)
            printf(" +sign");
        if (stackDrop != 0)
        ...
        for (auto& [a, p] : info->code)
        {
            ...
            if (p->instr->mId == X86_INS_RET)
            {
                if (p->stack + stackOffset != 0 && !isolate)
                    p->stop = utils::format("stack_unbalanced, %d/%d", info->func.stackDrop, -(p->stack + stackOffset));
            }
            if (p->instr->mId == X86_INS_RETF)
            {
                if (p->stack + stackOffsetRetf != 0 && !isolate)
                    p->stop = utils::format("stack_unbalanced, %d/%d", info->func.stackDrop, -(p->stack + stackOffsetRetf));
            }
        }
                    ...
                    if (!flag->provides.empty())
                    {
                        switch (flag->type)
                        {
                            ...
                            case 's':
                                if (p->instr->IsDirectCall())
                                {
                                    r = (procRequest_t)((int)r | (int)procRequest_t::returnSign);
                                    flag->visible = true;
                                }
                                else
                                ...
                                break;
                            default:
                                { fprintf(stderr, "analyser assert skipped at line %d\n", __LINE__); }
                        }
                    }
                }
                if (p->instr->IsDirectCall())
...
protected:
    ...
    std::map<address_t, int> tempIndexPrecond, tempIndexZf, tempIndexCf, tempIndexSf, tempIndexOf, tempIndexPf;
