...
//        .simpleJump = true,
    ...
    [X86_INS_SAHF] = { .savedVisiblyCarry = true, .savedVisiblyZero = true, .savedVisiblySign = true, .savedVisiblyParity = true },
    ...
    bool IsDirectCall()
    {
        ...
        if(mId == X86_INS_LCALL)
        {
            ...
            if (mDetail.op_count == 1 && mDetail.operands[0].type == X86_OP_MEM)
                return false;
            { fprintf(stderr, "tracer.h assert skipped\n"); };
        }
        { fprintf(stderr, "tracer.h assert skipped\n"); };
        return false;
    }
...
//         REPNE MOVSB f2 a4
        if (!strstr(mMnemonic, "rep"))
        {
            ...
            // else: f2/f3 prefix used as SSE/SSE2 mandatory prefix (not REP), ignore
        }
    }
}
...
//        cs_err err = cs_open(CS_ARCH_X86, CS_MODE_32, &mHandle);
    ...
    std::shared_ptr<CapInstr> Disasm(address_t addr)
    {
        ...
        if (!buf || (buf[0] == 0 && buf[1] == 0 && buf[2] == 0))
        {
    ...
    void Preprocess(cs_insn* insn)
    {
        if (mOptions->optStaticIndirectCall)
        {
                ...
                detail->operands[0].mem.base == X86_REG_INVALID)
            {
                ...
                const uint8_t* bp = GetBufferAt({mOptions->optStaticIndirectCallDs, (int)detail->operands[0].mem.disp});
    ...
    void Trace(address_t a)
    {
        ...
        while (!queue.empty())
        {
            ...
            for (std::pair<address_t, address_t> addr : queue)
            {
                ...
                if (mOptions.blackList.find(addr.second) != mOptions.blackList.end())
                    continue;
                std::shared_ptr<CapInstr> instr = Capstone->Disasm(addr.second);
                if (!instr) {
                    fprintf(stderr, "Warning: cannot disassemble at 0x%x (outside loaded sections)\n", addr.second.offset);
                    continue;
                }
