...
bool ConfigParser::ParseConfiguration(const std::vector<uint8_t>& fileContents, std::shared_ptr<Options> options)
{
    ...
    std::string strJson((const char*)&fileContents[0], fileContents.size());

    for (std::string line : utils::split(strJson.c_str(), "\n"))
    {
...
void ConfigParser::ParseConfigObject(CJson& json, std::shared_ptr<Options> options)
{
    json.ForEach([&](const CSubstring& k, const CSubstring& v){
        ...
        else if (k == "quiet" && v == "true")
            options->quiet = true;
        else if (k == "quiet" && v == "false")
            options->quiet = false;
        ...
        else if (k == "preprocess" && v == "false")
            options->preprocess = false;
        else if (k == "preprocess" && v == "true")
            options->preprocess = true;
        else if (k == "stackGuard" && v == "true")
            options->stackGuard = true;
        else if (k == "stackGuard" && v == "false")
            options->stackGuard = false;
        else if (k == "verboseIndirectCall" && v == "true")
            options->verboseIndirectCall = true;
        else if (k == "verboseIndirectCall" && v == "false")
            options->verboseIndirectCall = false;
        ...
        else if (k == "blackList")
        {
            ...
            {
                options->blackList.insert(address_t::fromString(CJson(v).GetString()));
            });
        }
        else if (k == "procList")
        {
            CJson(v).ForEach([&](const CSubstring& v)
            {
                ...
                if (strAddr.find("+") != std::string::npos)
                {
                    ...
                    auto callType = (options->arch == arch_t::arch32) ? procRequest_t::callLong : procRequest_t::callNear;
                    if (modifier == "+returnZero")
                        req = procRequest_t((int)procRequest_t::returnZero | (int)callType);
                    else if (modifier == "+returnCarry+returnZero")
                        req = procRequest_t((int)procRequest_t::returnZero | (int)procRequest_t::returnCarry | (int)callType);
                    else
                        assert(0);
