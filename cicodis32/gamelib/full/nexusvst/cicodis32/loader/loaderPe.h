...
public:
    ...
    virtual bool LoadFile(const char* filename, int loadAddress) override {
        ...
        mImageBase = loadAddress ? loadAddress : opt->ImageBase;
...
//            printf("Section %s va %x fsz %x vsz %x at %x\n", sec.name.c_str(), sec.vaddr, sh->SizeOfRawData, sec.vsize, sh->PointerToRawData);
        ...
        // Apply PE relocations when loading at a different base address
        uint32_t origBase = opt->ImageBase;
        if (mImageBase != origBase) {
            int32_t delta = (int32_t)(mImageBase - origBase);
            // Find .reloc section
            for (const auto& sec : mSections) {
                if (sec.name == ".reloc") {
                    const uint8_t* reloc = sec.data.data();
                    uint32_t pos = 0, fixed = 0;
                    while (pos + 8 <= sec.data.size()) {
                        uint32_t pageRVA   = *(uint32_t*)(reloc + pos);
                        uint32_t blockSize = *(uint32_t*)(reloc + pos + 4);
                        if (blockSize == 0 || blockSize < 8) break;
                        uint32_t numEntries = (blockSize - 8) / 2;
                        for (uint32_t i = 0; i < numEntries; i++) {
                            uint16_t entry = *(uint16_t*)(reloc + pos + 8 + i * 2);
                            if ((entry >> 12) != 3) continue; // IMAGE_REL_BASED_HIGHLOW only
                            uint32_t rva = pageRVA + (entry & 0xFFF);
                            // Find which section contains this RVA and patch it
                            for (auto& s : mSections) {
                                if (rva >= s.vaddr && rva + 4 <= s.vaddr + s.data.size()) {
                                    uint32_t offset = rva - s.vaddr;
                                    uint32_t val = *(uint32_t*)(s.data.data() + offset);
                                    val += delta;
                                    *(uint32_t*)(s.data.data() + offset) = val;
                                    fixed++;
                                    break;
                                }
                            }
                    ...
                    printf("// Applied %d PE relocations (delta=0x%x)\n", fixed, (uint32_t)delta);
                    break;
                }
            }
        }
 ...
 */
    ...
    virtual std::string GetInit() override {
        ...
        std::string exportsInfo;
        for (auto& is : ListExports())
            if (is.name != "main") // bug, points to wrong address
                exportsInfo += format("\nvoid %s()\n{\n    sub_%x();\n}\n\n", is.name.c_str(), is.rva + mImageBase);
//            exportsInfo += format("// Exports: %s at 0x%x, ordinal %d\n", is.name.c_str(), is.rva, is.ordinal);
        
        //printf("%s", exportsInfo.c_str());
...
//            *((uint32_t*)GetBufferAt(addr)) = ++symAddr;
        ...
        return includes + format("\nvoid sub_%x();\n\nvoid init()\n{\n%s    esp = 0x%x;\n    ebp = 0x%x;\n}\n\nvoid start()\n{\n    sub_%x();\n}\n",
                      ...
                      mImageBase + mSizeOfImage,
                      mImageBase + mEntryPoint) + exportsInfo + indirectEnum +
            ...
            if (is.byOrdinal)
            {
                if (is.dll != lastOrdinal)
                {
                    ...
                    if (!d.LoadFile(is.dll))
                        printf("// Missing ordinal imports for %s!\n", is.dll.c_str());
...
//                assert(lastOrdinals.size());
                std::string symbol;
                if (!lastOrdinals.empty()) {
                    std::map<int, std::string>::iterator it;
                    it = lastOrdinals.find(is.ordinal);
                    assert(it != lastOrdinals.end());
                    
                    symbol = it->second;
                } else {
                    symbol = format("ordinal_%d", is.ordinal);
                }
...
private:
    ...
    void PrintRTTI()
    {
        ...
        printf("\n// RTTI Information (%zu classes with vtables):\n", rttiInfo.size());
        printf("// ================================================================\n");
