...
inline OperandBuilder OP_X86(shared<CapInstr> instr, int index) {
    if (index < 0 || index >= instr->mDetail.op_count) {
        fprintf(stderr, "\nFATAL: Cannot decode operand %d of instruction '%s %s' at 0x%x (op_count=%d).\n"
                "The function containing this address is likely not a valid entry point (mid-instruction start).\n"
                "Fix: add the function address to blackList, e.g.:\n"
                "  {\"id\": \"config\", \"blackList\": [\"0:%x\"]}\n\n",
                index, instr->mMnemonic, instr->mOperands,
                instr->mAddress.linearOffset(), (int)instr->mDetail.op_count,
                instr->mAddress.linearOffset());
        abort();
    }
