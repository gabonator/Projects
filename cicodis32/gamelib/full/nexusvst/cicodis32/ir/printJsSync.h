...
public:
    ...
    virtual void PrintProgram(shared<ProcIr> prog, shared<Analyser::info_t> funcInfo) override
    {
        ...
        // SEH ESP check: save ESP before synthetic push, verify at every return
        bool hasSEH = prog->info.find("SEH prologue") != std::string::npos;
        // Scan for SEH prologue comment in first few lines
        for (size_t si = 0; si < std::min(prog->lines.size(), (size_t)10); si++)
            if (prog->lines[si].comment.find("SEH prologue") != std::string::npos)
                { hasSEH = true; break; }

        int stackDrop = funcInfo ? funcInfo->func.stackDrop : 0;

        // Bare function name without "()"
        std::string bareName = prog->name;
        if (bareName.size() >= 2 && bareName.substr(bareName.size()-2) == "()")
            bareName = bareName.substr(0, bareName.size()-2);

        if (hasSEH)
            printf("    var _espEntry = r32[esp];\n");
...
//                printf("  case 0x%x:\n", prog->addr.linearOffset()); //l.address.linearOffset()); TODO
            ...
            // Inject ESP check+fix before every return in SEH functions
            if (hasSEH && l.type == StatementIr::Type_t::Return)
            {
                printf("    if (r32[esp] !== (_espEntry + %d) >>> 0) "
                       "{ var _d=((r32[esp]-_espEntry-%d)|0); "
                       "if(!%s._espWarn){%s._espWarn=1; "
                       "console.log(\"%s: ESP fix delta=\"+_d);} "
                       "r32[esp] = (_espEntry + %d) >>> 0; }\n",
                       stackDrop, stackDrop,
                       bareName.c_str(), bareName.c_str(),
                       bareName.c_str(), stackDrop);
            }
        ...
        // For non-label functions (no while/switch), add check at the end
        if (hasSEH && !prog->labels)
        {
            printf("    if (r32[esp] !== (_espEntry + %d) >>> 0) "
                   "{ var _d=((r32[esp]-_espEntry-%d)|0); "
                   "if(!%s._espWarn){%s._espWarn=1; "
...
function start()
{
    ...
    {
        // TODO: ugly
        if (str == "edx" || str == "eax" || str == "ecx" || str == "ebx" ||
            str == "esi" || str == "edi" || str == "ebp" || str == "esp")
