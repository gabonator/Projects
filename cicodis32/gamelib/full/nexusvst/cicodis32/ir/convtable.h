...
std::function<StatementIr(convert_args)> convertir[X86_INS_ENDING] = {
    ...
    [X86_INS_LOOP] = [](convert_args){
        ...
        if(func.arch == arch_t::arch16)
            ...
            fprintf(stderr, "convtable.h assert skipped at line %d\\n", __LINE__);
    ...
    [X86_INS_RETF] = [](convert_args){
        ...
        if (func.callConv == callConv_t::callConvShiftStackFar)
            shift += 2; //fprintf(stderr, "convtable.h assert skipped at line %d\\n", __LINE__); //aux.push_back("sp += 2;");  // -- TODO POP CS????
        if (func.callConv == callConv_t::callConvShiftStackNearFar)
            shift += 2; //fprintf(stderr, "convtable.h assert skipped at line %d\\n", __LINE__); //aux.push_back("sp += 2;");
    ...
    // SETcc — handled by simpleCond/conditionAs in tracer.h (maps to Jcc conditions)
    // Remaining SETcc that don't have simpleCond mappings:
    [X86_INS_SETO]  = [](convert_args){ return ASSIGN(OP_X86(instr, 0), OP_VAR("flags.overflow ? 1 : 0")); },
    [X86_INS_SETNO] = [](convert_args){ return ASSIGN(OP_X86(instr, 0), OP_VAR("flags.overflow ? 0 : 1")); },
    [X86_INS_SETB]  = [](convert_args){ return ASSIGN(OP_X86(instr, 0), OP_VAR("flags.carry ? 1 : 0")); },
    [X86_INS_SETAE] = [](convert_args){ return ASSIGN(OP_X86(instr, 0), OP_VAR("flags.carry ? 0 : 1")); },
    [X86_INS_SETBE] = [](convert_args){ return ASSIGN(OP_X86(instr, 0), OP_VAR("(flags.carry || flags.zero) ? 1 : 0")); },
    [X86_INS_SETA]  = [](convert_args){ return ASSIGN(OP_X86(instr, 0), OP_VAR("(!flags.carry && !flags.zero) ? 1 : 0")); },
    [X86_INS_SETS]  = [](convert_args){ return ASSIGN(OP_X86(instr, 0), OP_VAR("flags.sign ? 1 : 0")); },
    [X86_INS_SETNS] = [](convert_args){ return ASSIGN(OP_X86(instr, 0), OP_VAR("flags.sign ? 0 : 1")); },
    [X86_INS_SETP]  = [](convert_args){ return ASSIGN(OP_X86(instr, 0), OP_VAR("flags.parity ? 1 : 0")); },
    [X86_INS_SETNP] = [](convert_args){ return ASSIGN(OP_X86(instr, 0), OP_VAR("flags.parity ? 0 : 1")); },
    ...
    [X86_INS_IMUL] = [](convert_args){
        ...
        if (instr->mDetail.op_count == 2 && instr->mDetail.operands[0].size == 2)
        {
            // imul r16, r/m16: r16 = r16 * r/m16 (signed, truncated to 16 bits)
            StatementIr st = OP_BINARY(OP_SIGNED(OP_X86(instr, 0)), "*", OP_SIGNED(OP_X86(instr, 1)));
            st.opd = OP_X86(instr, 0).get();
            return st;
        }
        ...
        fprintf(stderr, "convtable.h: unhandled IMUL form op_count=%d size=%d\n", instr->mDetail.op_count, instr->mDetail.operands[0].size);
        return StatementIr{.type = StatementIr::Type_t::Stop, .stop = "unhandled IMUL form"};
    },
    ...
    [X86_INS_XCHG] = [](convert_args){
        ...
        switch (tempSize)
        {
            ...
            default: fprintf(stderr, "convtable.h assert skipped at line %d\\n", __LINE__);
        }
    ...
    [X86_INS_FSUBR] = [](convert_args) -> StatementIr {
        if (instr->mDetail.op_count == 2 && instr->mDetail.operands[0].type == X86_OP_REG) {
            int index0 = instr->mDetail.operands[0].reg - X86_REG_ST0;
            int index1 = instr->mDetail.operands[1].reg - X86_REG_ST0;
            return OP_FUNCTION("fsubrst2", OP_CONST(index0), OP_CONST(index1));
        }
        if (instr->mDetail.op_count == 1 && instr->mDetail.operands[0].type == X86_OP_REG) {
            int index0 = instr->mDetail.operands[0].reg - X86_REG_ST0;
            return OP_FUNCTION("fsubrst", OP_CONST(index0));
        }
        if (instr->mDetail.op_count != 1) {
    ...
    // FUCOM/FUCOMP/FUCOMPP — unordered compare (same as FCOM but no exception on NaN)
    // NOTE: These are correctly implemented but cause FPU state divergence issues
    // during plugin init. Keep them enabled — the divergence must be fixed upstream.
    [X86_INS_FUCOM] = [](convert_args){
        if (instr->mDetail.op_count == 1 && instr->mDetail.operands[0].type == X86_OP_REG) {
            int index0 = instr->mDetail.operands[0].reg - X86_REG_ST0;
            return OP_FUNCTION("fcomst", OP_CONST(index0));
        }
        if (instr->mDetail.op_count == 0)
            return OP_FUNCTION("fcomst", OP_CONST(1));
        return OP_FUNCTION("fcom#", OP_X86(instr, 0));
    },
    [X86_INS_FUCOMP] = [](convert_args){
        if (instr->mDetail.op_count == 1 && instr->mDetail.operands[0].type == X86_OP_REG) {
            int index0 = instr->mDetail.operands[0].reg - X86_REG_ST0;
            return OP_FUNCTION("fcompst", OP_CONST(index0));
        }
        if (instr->mDetail.op_count == 0)
            return OP_FUNCTION("fcompst", OP_CONST(1));
        return OP_FUNCTION("fcomp#", OP_X86(instr, 0));
    },
    [X86_INS_FUCOMPP] = [](convert_args){ return OP_FUNCTION("fcompp"); },
    [X86_INS_FUCOMI] = [](convert_args){
        if (instr->mDetail.op_count >= 1 && instr->mDetail.operands[0].type == X86_OP_REG) {
            int index0 = instr->mDetail.operands[0].reg - X86_REG_ST0;
            return OP_FUNCTION("fcomst", OP_CONST(index0));
        }
