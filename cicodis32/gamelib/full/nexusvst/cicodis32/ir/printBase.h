...
public:
    ...
    virtual void PrintProgram(shared<ProcIr> prog, shared<Analyser::info_t> info)
    {
        ...
        printf("{\n");
        if (info && info->func.options && info->func.options->stackGuard)
            printf("    StackGuard _sg(%d, __FUNCTION__);\n", info->func.stackDrop);
...
//            suffix = format("%d", st.suffix);
        ...
        switch (st.type)
        {
            ...
            case StatementIr::Type_t::Unary:
                ...
                if (st.stmt1)
                {
                    // Inner ToString may emit trailing ";" for function calls — strip it
                    std::string inner = trim(replace(ToString(*st.stmt1), ");", ")"));
                    if (inner.starts_with("flags.") || inner.starts_with("!")) // TODO: ugly
                        return format("%s%s", st.oper.c_str(), inner.c_str());
                    else
                        return format("%s(%s)", st.oper.c_str(), inner.c_str());
                }
                else if (st.opin1)
...
//                return SignedType(st.opin1);
            ...
            case StatementIr::Type_t::Condition:
            {
                ...
                // Function calls and stop() emit trailing ";" — strip it when used as condition
            ...
            case StatementIr::Type_t::IndirectCall:
                assert(st.opin1);
                if (mOptions->verboseIndirectCall)
                    return format("    indirectCall(cs, %s, 0x%04x, 0x%08x); // %s", ToString(st.opin1).c_str(), st.address.segment, st.address.offset, st.address.toString().c_str());
            ...
            case StatementIr::Type_t::IndirectCallLong:
                ...
                if (mOptions->verboseIndirectCall)
                    return format("    push(cs); cs = %s; indirectCall(cs, %s, 0x%04x, 0x%08x); assert(cs == 0x%04x); // %s", ToString(st.opin1).c_str(), ToString(st.opin2).c_str(), st.address.segment, st.address.offset, st.address.segment, st.address.toString().c_str());
            ...
            case StatementIr::Type_t::IndirectJump:
                assert(st.opin1 && st.opin2);
                return format("    indirectJump(%s, %s, 0x%04x, 0x%08x); return; // %s", ToString(st.opin1).c_str(), ToString(st.opin2).c_str(), st.address.segment, st.address.offset, st.address.toString().c_str());
            ...
            case StatementIr::Type_t::Switch:
            {
                ...
                aux += "        default:\n";
//                aux += format("            callByAddressOrIAT(%s); break; // fallback dispatch at %s\n", sel.c_str(), st.address.toString().c_str());
                aux += format("            stop(\"unhandled indirect %s\");\n", st.address.toString().c_str()); // TODO
