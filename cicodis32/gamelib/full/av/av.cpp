...
void sub_1ed0() // +entry
{
    ...
    sub_2025();
    di = memoryAGet16(ds, 0x007e); es = memoryAGet16(ds, 0x007e + 2);
    ax = di;
    bx = ax;
    cx = 0x7fff;
loc_1f0a:
...
loc_1f98:
    ...
    sub_2041();
    push(cs);
    indirectCall(cs, memoryAGet16(ds, 0x095a)); // 01ed:011e;
    ax = 0;
    si = ax;
    cx = 0x002f;
    flags.direction = 0;
...
loc_205d:
    ...
    push(ax);
    sub_1fe5();
    stop("mark 01ed:01a3, addr 01ed:01a0, instr: call 0x115, bytes: e8 72 ff", "01ed:01a0");
    memoryASet(ds, bx + si, memoryAGet(ds, bx + si) + al);
    stop("mark 01ed:01a3, addr 01ed:01a3, instr: add byte ptr [bx + si], al, bytes: 00 00mark 01ed:01a4, addr 01ed:01a3, instr: add byte ptr [bx + si], al, bytes: 00 00", "01ed:01a3");
    push(si);
    push(di);
    push(bp);
    bp = sp;
...
void sub_2041()
{
    ...
    push(ds);
    ax = 0x2500;
    dx = memoryAGet16(ds, 0x0074); ds = memoryAGet16(ds, 0x0074 + 2);
    interrupt(0x21);
    ds = pop();
    sp += 2;
}
...
loc_40b5:
    ...
    bx = ax;
    bx <<= 1;
    indirectJump(cs, memoryAGet16(cs, bx + 8707)); return; // 01ed:21fe;
    // gap 33 bytes
loc_40f4:
    ax = memoryAGet16(ss, bp - 2);
    ax--;
    ...
    bx = ax;
    bx <<= 1;
    indirectJump(cs, memoryAGet16(cs, bx + 8758)); return; // 01ed:2231;
    // gap 35 bytes
loc_4129:
    ax = 0x0001;
    ax -= si;
...
loc_5bdb:
    ...
    bx = ax;
    bx <<= 1;
    indirectJump(cs, memoryAGet16(cs, bx + 15654)); return; // 01ed:3d21;
    // gap 74 bytes
loc_5c40:
...
loc_5f02:
    ...
    bx <<= 1;
    indirectCall(cs, memoryAGet16(ds, bx + 2642)); // 01ed:4043;
    goto loc_5f02;
loc_5f19:
    indirectCall(cs, memoryAGet16(ds, 0x03e2)); // 01ed:4049;
    indirectCall(cs, memoryAGet16(ds, 0x03e4)); // 01ed:404d;
    indirectCall(cs, memoryAGet16(ds, 0x03e6)); // 01ed:4051;
    push(memoryAGet16(ss, bp + 4));
    sub_1fe5();
    cx = pop();
    bp = pop();
...
//    push(cs); cs = memoryAGet16(ss, bp - 14 + 2); indirectCall(cs, memoryAGet16(ss, bp - 14)); assert(cs == 0x01ed); // 01ed:42b1;
    ...
    flags.zero = !(memoryAGet16(ds, si + 12) & 0x0001);
...
void sub_653c() // +nearfar +stackDrop10
{
    sp -= 2;
    push(bp);
    bp = sp;
...
void sub_704a()
{
    ...
    si = memoryAGet16(ss, bp + 12); ds = memoryAGet16(ss, bp + 12 + 2);
    ax = ds;
    ax |= si;
    if (!ax)
        goto loc_7063;
...
void sub_7091()
{
    ...
    bp = sp;
    push(ds);
    dx = memoryAGet16(ss, bp + 14); ds = memoryAGet16(ss, bp + 14 + 2);
    ax = 0x3d00;
    interrupt(0x21);
    ds = pop();
    if (!flags.carry)
...
void sub_753d()
{
    ...
    bx = ax;
    bx <<= 1;
    switch (memoryAGet16(cs, bx + 22160))
    {
        case 0x56a4: goto loc_7574;
        default:
            stop("ind 01ed:568b");
    }
    // gap 20 bytes
...
void sub_7813()
{
    ...
    bx = ax;
    bx <<= 1;
    indirectJump(cs, memoryAGet16(cs, bx + 22895)); return; // 01ed:596a;
    // gap 78 bytes
loc_788d:
    if (si == 0x0001)
        goto loc_7897;
...
loc_7ac3:
    ...
    goto loc_7aff;
loc_7ac8:
    bx = memoryAGet16(ds, 0x0b5d); es = memoryAGet16(ds, 0x0b5d + 2);
...
void sub_7b86()
{
    ...
    if (ax)
        goto loc_7bb0;
    bx = memoryAGet16(ds, 0x0412); es = memoryAGet16(ds, 0x0412 + 2);
...
void sub_7f05() // +nearfar +stackDrop2
{
    sp -= 2;
    push(ds);
    push(si);
...
loc_84ff:
    bx = memoryAGet16(ds, di + 4); es = memoryAGet16(ds, di + 4 + 2);
...
void sub_85c1()
{
    ...
    bx = memoryAGet16(ds, 0x0416); es = memoryAGet16(ds, 0x0416 + 2);
    si = 0x0000;
    switch (memoryAGet16(ds, 0x0412 + 2)*0x10000 + memoryAGet16(ds, 0x0412))
    {
...
void sub_89d4() // +nearfar +stackDrop2
{
    sp -= 2;
    push(bp);
    bp = sp;
...
void sub_8b12() // +nearfar +stackDrop2
{
    sp -= 2;
    push(bp);
    bp = sp;
...
void sub_8b48()
{
    ...
    push(si);
    push(di);
    bx = memoryAGet16(ds, 0x0609); es = memoryAGet16(ds, 0x0609 + 2);
...
{
    sp -= 2;
    stop("mark 043e:0010, addr 043e:0010, instr: ret , bytes: c3");
    sp += 2;
}
void sub_43f5()
{
...
loc_5232:
    ...
    if (memoryAGet(cs, 0x010b) < 0x04)
        goto loc_5250;
    push(es);
    indirectCall(cs, memoryAGet16(cs, 0x051e)); // 043e:0e66;
    push(es);
    ds = pop();
    es = pop();
    goto loc_5255;
...
loc_5311:
    ...
    ax = pop();
    tx = di; di = bx; bx = tx;
    indirectCall(cs, memoryAGet16(cs, 0x051e)); // 043e:0f3e;
    tx = di; di = bx; bx = tx;
    memoryASet16(cs, 0x0e12, memoryAGet16(cs, 0x0e12) - 1);
    if (memoryAGet16(cs, 0x0e12))
        goto loc_5311;
...
loc_4aaf:
    if (dh & 0x80)
        goto loc_4abc;
    dx += di;
    indirectCall(cs, memoryAGet16(ds, 0x051e)); // 043e:06d6;
    goto loc_4abe;
loc_4abc:
    dx += si;
loc_4abe:
...
loc_4ce1:
    ...
    dl >>= 1;
loc_4cf6:
    temp_cond0 = (char)dh >= (char)dl /*xxx*/;
    dx = pop();
    if (temp_cond0)
        goto loc_4d05;
    indirectCall(cs, memoryAGet16(cs, 0x07cf)); // 043e:091b;
    di++;
    al = ah;
    bl = 0xff;
loc_4d05:
...
loc_4f02:
    push(es);
    indirectCall(cs, memoryAGet16(cs, 0x051e)); // 043e:0b23;
    memoryASet(cs, 0x0a43, al);
    ax = es;
    ds = ax;
    es = pop();
...
loc_533a:
    ...
    if (!dx)
        goto loc_5350;
    indirectCall(cs, memoryAGet16(cs, 0x0e9f)); // 043e:0f60;
    bl = 0xff;
    di++;
    ah = ch;
    al = lodsb<DS_SI>();
...
loc_5371:
    ...
    if (temp_cond0)
        goto loc_5384;
    indirectCall(cs, memoryAGet16(cs, 0x0e9f)); // 043e:0f96;
    di++;
    ah = ch;
    al = 0;
    ax >>= cl;
...
loc_5384:
    bl &= dl;
    indirectCall(cs, memoryAGet16(cs, 0x0e9f)); // 043e:0fa6;
    sp += 2;
}
void sub_538c()
{
...
void sub_53bc()
{
    ...
    bh = 0;
    bl <<= 1;
    indirectCall(cs, memoryAGet16(ds, bx + 4046)); // 043e:0fec;
    dl = al;
    dh = 0;
    sp += 2;
}
...
loc_53e3:
    ...
    bh = 0;
    bl <<= 1;
    indirectJump(cs, memoryAGet16(ds, bx + 4032)); return; // 043e:1013;
}
...
void sub_565b()
{
    ...
    if (bx >= memoryAGet16(ds, 0x0dea))
        goto loc_566d;
    indirectCall(cs, memoryAGet16(ds, 0x0de2)); // 043e:1281;
    sub_58ae();
    ch = 0x80;
    sub_57a1();
loc_566d:
...
void sub_566e()
{
    ...
    if (bx == memoryAGet16(ds, 0x0de8))
        goto loc_5683;
    indirectCall(cs, memoryAGet16(ds, 0x0de0)); // 043e:1297;
    sub_58ae();
    ch = 0;
    sub_57a1();
loc_5683:
...
void sub_8f4c()
{
    ...
    push(dx);
    si = 0x0008;
    push(cs); cs = memoryAGet16(ds, 0x0049 + 2); indirectCall(cs, memoryAGet16(ds, 0x0049)); assert(cs == 0x08e6); // 08e6:0101;
    bx = pop();
    ax = pop();
    sp += 2;
    return;
...
loc_9037:
    ...
    switch (memoryAGet16(ds, 0x0049 + 2)*0x10000 + memoryAGet16(ds, 0x0049))
    {
        case 0x43e0000: push(cs); cs = 0x043e; sub_43e0(); assert(cs == 0x08e6); break;
        default:
            stop("ind 08e6:01df");
    }
    ...
    push(es);
    push(bx);
    di = memoryAGet16(ds, 0x0049); es = memoryAGet16(ds, 0x0049 + 2);
    di += 0x0010;
    al = 0x9a;
    flags.direction = 0;
    stosb<ES_DI>(al);
    ...
    ax = cs;
    stosw<ES_DI>(ax);
    si = memoryAGet16(cs, 0x004d); es = memoryAGet16(cs, 0x004d + 2);
    si += 0x0002;
    ax = memoryAGet16(es, si);
...
void sub_90c8()
{
    ...
    si = 0x001e;
    push(cs); cs = memoryAGet16(ds, 0x0049 + 2); indirectCall(cs, memoryAGet16(ds, 0x0049)); assert(cs == 0x08e6); // 08e6:026e;
    sp += 2;
}
void sub_90d3()
{
...
loc_9100:
    ...
    cx = 0x0001;
    si = 0x0022;
    push(cs); cs = memoryAGet16(ds, 0x0049 + 2); indirectCall(cs, memoryAGet16(ds, 0x0049)); assert(cs == 0x08e6); // 08e6:02ad;
    sp += 2;
}
void sub_9112()
{
...
loc_929b:
    ...
    al <<= 1;
    di = ax;
    indirectJump(cs, memoryAGet16(ds, di + 1093)); return; // 08e6:0441;
}
...
void sub_a231()
{
    ...
    ax <<= 1;
    ax += 0x0010;
    di = memoryAGet16(ds, 0x1311); ds = memoryAGet16(ds, 0x1311 + 2);
    di += ax;
loc_a24a:
    al = memoryAGet(es, bx);
    bx++;
...
void sub_a273()
{
    ...
    al <<= 1;
    ah = 0;
    di = memoryAGet16(ds, 0x1311); es = memoryAGet16(ds, 0x1311 + 2);
    bx = di;
    bx += 0x0010;
    bx += ax;
...
loc_a2a5:
    ...
    ah = 0;
    di = ax;
    indirectJump(cs, memoryAGet16(ds, di + 5204)); return; // 08e6:1450;
    // gap 101 bytes
loc_a319:
    push(es);
    push(di);
...
loc_a322:
    ...
    ah = 0;
    di = ax;
    indirectCall(cs, memoryAGet16(ds, di + 5333)); // 08e6:14cd;
    di = pop();
    es = pop();
    goto loc_a322;
}
...
{
    sp -= 2;
    si = memoryAGet16(ds, 0x1311); ds = memoryAGet16(ds, 0x1311 + 2);
    cx = 0x0010;
    di = 0x12fd;
    push(cs);
    es = pop();
...
void fixReloc(uint16_t seg)
{
    memoryASet16(seg, 0x0002, memoryAGet16(seg, 0x0002) + seg); // 086a -> 0a57; lin=2
    memoryASet16(seg, 0x0459, memoryAGet16(seg, 0x0459) + seg); // 0000 -> 01ed; lin=459
    memoryASet16(seg, 0x038a, memoryAGet16(seg, 0x038a) + seg); // 0000 -> 01ed; lin=38a
    memoryASet16(seg, 0x02f8, memoryAGet16(seg, 0x02f8) + seg); // 0000 -> 01ed; lin=2f8
    memoryASet16(seg, 0x01e7, memoryAGet16(seg, 0x01e7) + seg); // 0000 -> 01ed; lin=1e7
    memoryASet16(seg, 0x01df, memoryAGet16(seg, 0x01df) + seg); // 0000 -> 01ed; lin=1df
    memoryASet16(seg, 0x099e, memoryAGet16(seg, 0x099e) + seg); // 0000 -> 01ed; lin=99e
    memoryASet16(seg, 0x097b, memoryAGet16(seg, 0x097b) + seg); // 0000 -> 01ed; lin=97b
    memoryASet16(seg, 0x0914, memoryAGet16(seg, 0x0914) + seg); // 0000 -> 01ed; lin=914
    memoryASet16(seg, 0x07ee, memoryAGet16(seg, 0x07ee) + seg); // 0000 -> 01ed; lin=7ee
    memoryASet16(seg, 0x0635, memoryAGet16(seg, 0x0635) + seg); // 0000 -> 01ed; lin=635
    memoryASet16(seg, 0x0621, memoryAGet16(seg, 0x0621) + seg); // 0000 -> 01ed; lin=621
    memoryASet16(seg, 0x0604, memoryAGet16(seg, 0x0604) + seg); // 0000 -> 01ed; lin=604
    memoryASet16(seg, 0x05e9, memoryAGet16(seg, 0x05e9) + seg); // 0000 -> 01ed; lin=5e9
    memoryASet16(seg, 0x05c9, memoryAGet16(seg, 0x05c9) + seg); // 0000 -> 01ed; lin=5c9
    memoryASet16(seg, 0x05b5, memoryAGet16(seg, 0x05b5) + seg); // 0000 -> 01ed; lin=5b5
    memoryASet16(seg, 0x0c67, memoryAGet16(seg, 0x0c67) + seg); // 086a -> 0a57; lin=c67
    memoryASet16(seg, 0x0c57, memoryAGet16(seg, 0x0c57) + seg); // 0000 -> 01ed; lin=c57
    memoryASet16(seg, 0x0c43, memoryAGet16(seg, 0x0c43) + seg); // 0000 -> 01ed; lin=c43
    memoryASet16(seg, 0x0c2f, memoryAGet16(seg, 0x0c2f) + seg); // 0000 -> 01ed; lin=c2f
    memoryASet16(seg, 0x0c09, memoryAGet16(seg, 0x0c09) + seg); // 0000 -> 01ed; lin=c09
    memoryASet16(seg, 0x0bf4, memoryAGet16(seg, 0x0bf4) + seg); // 0000 -> 01ed; lin=bf4
    memoryASet16(seg, 0x0b95, memoryAGet16(seg, 0x0b95) + seg); // 0000 -> 01ed; lin=b95
    memoryASet16(seg, 0x0b4d, memoryAGet16(seg, 0x0b4d) + seg); // 0000 -> 01ed; lin=b4d
    memoryASet16(seg, 0x0b2c, memoryAGet16(seg, 0x0b2c) + seg); // 0000 -> 01ed; lin=b2c
    memoryASet16(seg, 0x0b0c, memoryAGet16(seg, 0x0b0c) + seg); // 0000 -> 01ed; lin=b0c
    memoryASet16(seg, 0x0ae8, memoryAGet16(seg, 0x0ae8) + seg); // 0000 -> 01ed; lin=ae8
    memoryASet16(seg, 0x0ac7, memoryAGet16(seg, 0x0ac7) + seg); // 0000 -> 01ed; lin=ac7
    memoryASet16(seg, 0x0aa7, memoryAGet16(seg, 0x0aa7) + seg); // 0000 -> 01ed; lin=aa7
    memoryASet16(seg, 0x0a15, memoryAGet16(seg, 0x0a15) + seg); // 0000 -> 01ed; lin=a15
    memoryASet16(seg, 0x1904, memoryAGet16(seg, 0x1904) + seg); // 0000 -> 01ed; lin=1904
    memoryASet16(seg, 0x18ee, memoryAGet16(seg, 0x18ee) + seg); // 0000 -> 01ed; lin=18ee
    memoryASet16(seg, 0x18e2, memoryAGet16(seg, 0x18e2) + seg); // 0000 -> 01ed; lin=18e2
    memoryASet16(seg, 0x18c5, memoryAGet16(seg, 0x18c5) + seg); // 0000 -> 01ed; lin=18c5
    memoryASet16(seg, 0x188f, memoryAGet16(seg, 0x188f) + seg); // 0000 -> 01ed; lin=188f
    memoryASet16(seg, 0x1862, memoryAGet16(seg, 0x1862) + seg); // 0000 -> 01ed; lin=1862
    memoryASet16(seg, 0x172f, memoryAGet16(seg, 0x172f) + seg); // 0000 -> 01ed; lin=172f
}

int GetProcAt(int seg, int ofs)
{
    int marks[] = {
0x01ed, 0x01a3, 0x01ed, 0x01a4, 0x043e, 0x0010,     };
    for (int i=0; i<sizeof(marks)/sizeof(marks[0]); i+=2)
        if (seg == marks[i] && ofs == marks[i+1])
            return 0;

    int map[] = {
        0x1ed0, 0x1ed, 0x0, 0x1ed, 0x14c, 0x1ed0, 0x1ed, 0x18d, 0x1ed, 0x1a0, 
        0x1ed0, 0x1ed, 0x1a5, 0x1ed, 0x2ff, 0x1fe5, 0x1ed, 0x115, 0x1ed, 0x14c, 
        ...
        0x2215, 0x1ed, 0x345, 0x1ed, 0x41a, 0x22ea, 0x1ed, 0x41a, 0x1ed, 0x43a, 
        ...
        0x329a, 0x1ed, 0x13ca, 0x1ed, 0x1452, 0x3322, 0x1ed, 0x1452, 0x1ed, 0x166f, 
        0x353f, 0x1ed, 0x166f, 0x1ed, 0x1747, 0x3617, 0x1ed, 0x1747, 0x1ed, 0x199b, 
        0x64c4, 0x1ed, 0x45f4, 0x1ed, 0x463b, 0x651e, 0x1ed, 0x464e, 0x1ed, 0x4668, 
        0x653c, 0x1ed, 0x466c, 0x1ed, 0x4698, 0x6568, 0x1ed, 0x4698, 0x1ed, 0x46ba, 
        0x658a, 0x1ed, 0x46ba, 0x1ed, 0x4822, 0x66f2, 0x1ed, 0x4822, 0x1ed, 0x485f, 
        0x672f, 0x1ed, 0x485f, 0x1ed, 0x4892, 0x6762, 0x1ed, 0x4892, 0x1ed, 0x4958, 
        0x6828, 0x1ed, 0x4958, 0x1ed, 0x4993, 0x6863, 0x1ed, 0x4993, 0x1ed, 0x49db, 
        0x68ab, 0x1ed, 0x49db, 0x1ed, 0x49f0, 0x68c6, 0x1ed, 0x190, 0x1ed, 0x1a0, 
        0x68c6, 0x1ed, 0x1a5, 0x1ed, 0x2ff, 0x68c6, 0x1ed, 0x49fb, 0x1ed, 0x4a7f, 
        0x68c6, 0x1ed, 0x4aaa, 0x1ed, 0x4ae1, 0x694f, 0x1ed, 0x4a7f, 0x1ed, 0x4aa7, 
        0x69b1, 0x1ed, 0x18d, 0x1ed, 0x1a0, 0x69b1, 0x1ed, 0x1a5, 0x1ed, 0x2ff, 
        ...
        0x6f15, 0x1ed, 0x5045, 0x1ed, 0x5068, 0x6f38, 0x1ed, 0x5068, 0x1ed, 0x50e0, 
        0x6fb0, 0x1ed, 0x50e0, 0x1ed, 0x50ea, 0x7010, 0x1ed, 0x5140, 0x1ed, 0x515f, 
        0x702f, 0x1ed, 0x515f, 0x1ed, 0x517a, 0x704a, 0x1ed, 0x517a, 0x1ed, 0x51a8, 
        0x7078, 0x1ed, 0x51a8, 0x1ed, 0x51b8, 0x7088, 0x1ed, 0x51b8, 0x1ed, 0x51c1, 
        0x7091, 0x1ed, 0x51c1, 0x1ed, 0x5208, 0x70d8, 0x1ed, 0x5208, 0x1ed, 0x5241, 
        0x726d, 0x1ed, 0x539d, 0x1ed, 0x53b5, 0x7285, 0x1ed, 0x53b5, 0x1ed, 0x53d8, 
        0x72a8, 0x1ed, 0x53d8, 0x1ed, 0x5409, 0x72d9, 0x1ed, 0x5409, 0x1ed, 0x5441, 
        0x7311, 0x1ed, 0x5441, 0x1ed, 0x547d, 0x734d, 0x1ed, 0x547d, 0x1ed, 0x549a, 
        ...
        0x753d, 0x1ed, 0x56a4, 0x1ed, 0x56ae, 0x753d, 0x1ed, 0x56e8, 0x1ed, 0x56fb, 
        0x75cb, 0x1ed, 0x56fb, 0x1ed, 0x5749, 0x7649, 0x1ed, 0x5779, 0x1ed, 0x57dc, 
        0x76cd, 0x1ed, 0x57fd, 0x1ed, 0x5844, 0x7714, 0x1ed, 0x5844, 0x1ed, 0x5943, 
        0x7813, 0x1ed, 0x5943, 0x1ed, 0x596f, 0x7813, 0x1ed, 0x59bd, 0x1ed, 0x59f2, 
        0x78c2, 0x1ed, 0x59f2, 0x1ed, 0x5aa2, 0x7972, 0x1ed, 0x5aa2, 0x1ed, 0x5b99, 
