//"use strict";
let cs=0, ds=1, es=2, ss=3, sp=4, fs=5;
...
const memorysize = 5*1024*1024
const memorybuf = new ArrayBuffer(memorysize);
const memory = new Uint8Array(memorybuf, 0, memorysize);
const memorys = new Int8Array(memorybuf, 0, memorysize);
const memoryView = new DataView(memorybuf);
let allocatorPtr = 0x103a0000; //0x10080000;
...
function memoryASet32(seg, ofs, v)
{
  ...
  if (!(ofs >= 0x1004d000 && ofs < 0x10000000 + memorysize)) { console.log("memoryASet32 bad ofs: 0x" + ofs.toString(16)); assert(false); }
  ofs -= 0x10000000;
  memory[ofs] = v;
  memory[ofs+1] = v >> 8;
  memory[ofs+2] = v >> 16;
...
{   
  ...
  if (!(ofs >= 0x10001000 && ofs < 0x10000000 + memorysize)) { console.log("memoryAGet32 bad ofs: 0x" + ofs.toString(16)); assert(false); }
  ofs -= 0x10000000;
...
{
  assert(ofs >= 0x10000000 + 0x1000 && ofs < 0x10000000 + memorysize);
  ofs -= 0x10000000;
  memory[ofs] = v & 0xff;
  memory[ofs+1] = v >> 8;
}
