...
<script src="https://code.highcharts.com/modules/exporting.js"></script>
    ...
    <script language = "javascript"type = "text/javascript">
      ...
      function testWebSocket() {
          ...
          websocket.onopen = function(evt) {
              ...
              getInfoTimer = setInterval(getInfo, 5000);
          };
      ...
      function changeOsc() {
        var value = document.getElementById("oscilloscope").checked;
          websocket.send("bOscilloscope="+value*1+"\n");
      }
      
      function changeLed() {
        var value = document.getElementById("led").checked;
          websocket.send("bLed="+value*1+"\n");
      }
      
      function changeDebug() {
        var value = document.getElementById("debug").checked;
          websocket.send("bDebug="+value*1+"\n");
      }
    ...
    <body>
        ...
        <div id="info"></div>
<canvas id="graf" width="600" height="200"></canvas>
<br>
    <input id="oscilloscope" type="checkbox" onchange="changeOsc();">Oscilloscope</input>
    <input id="led" type="checkbox" onchange="changeLed();">Led</input>
    <input id="debug" type="checkbox" onchange="changeDebug();">Debug</input>
<input type="text" id="cmd"><input value="send" type="button"
  onClick="websocket.send(document.getElementById('cmd').value)"><br>
<div id="con"></div>
...
var lyo = 0;
function debug(threshHigh, threshLow, out)
{
  var y = (1-parseInt(threshHigh)/1024)*can.height;
  ...
  var y = (1-parseInt(threshLow)/1024)*can.height;
...
function updatesecond()
{
  ...
  ctx.beginPath();
  ctx.moveTo(x, can.height*0.05);
  ctx.lineTo(x, can.height*0.10);
  ctx.stroke();
...
function updatepulse(count)
{
  ...
  {
    ctx.beginPath();
    ctx.moveTo(x - i*10, can.height*0.00);
    ctx.lineTo(x - i*10, can.height*0.05);
    ctx.stroke();
    onPulse();
  }
}
...
var _h = {
    ...
    debug: (a, b, c) => debug(a, b, c),
    second: () => updatesecond(),
    pulse: (count) => updatepulse(count),
    log: (msg) => console.log(msg),
    scan: {
      started: () => console.log("Inquiry started..."),
      finished: () => console.log("Inquiry finished..."),
      onNetwork: (ssid, rssi) => console.log(ssid + ": "+rssi+" dB")
    },
    //onRssi: (ssid, rssi) => console.log(ssid + ": "+rssi+" dB")
    onStatus: (ssid, rssi, name) => {
      clearTimeout(getInfoClearTimer);
      document.getElementById("info").innerHTML = "Ssid: "+ssid+" Rssi:"+rssi+" dB Info:"+name;
    }
};

setInterval(()=>
{
    if (online && !document.getElementById("oscilloscope").checked)
    {
        ctx.fillStyle = "#000000";
        ctx.fillRect(x+8, 0, 1, can.height);
        ctx.fillStyle = "rgba(200, 2255, 200, 1)";
        ctx.fillRect(x+1, 0, 4, can.height);
        x++;
        if (x>can.width)
            x = 0;        
...
function getInfo()
{
  ...
  clearTimeout(getInfoClearTimer);
  getInfoClearTimer = setTimeout(getInfoClear, 2000);
}

function getInfoClear()
{
